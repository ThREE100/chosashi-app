## 【土地家屋調査士受験生向け】平成28年度 第13問〜建物を分けても、抵当権は消えない〜
...
- 各肢の法的根拠（ア：抵当権名義人の承諾による存続、イ：共用部分の登記がある場合の所有者を証する情報、ウ：代位による分割登記の申請、エ：不動産所在事項変更の記録方法、オ：所有権登記の転写要件）は、土地家屋調査士試験対策アプリの検証済みデータベースのexplanationフィールド（および補足のpitfalls・keyPointsフィールド）を出発点としつつ、条文レベルで裏付けを取り直したものです。**条文原文で確認できた根拠**：ア（不動産登記法54条3項が40条を準用する点）、イ（同法54条2項・58条4項）、エ（不動産登記規則127条3項の「変更後の不動産所在事項、分割により変更した旨及び変更前の不動産所在事項を抹消する記号」という文言）、オ（同規則128条2項の「転写することに代えて……分割による所有権の登記をする旨」という文言）。**条文の明文までは確認できていない根拠**：ウ（附属建物の取得者による代位申請の可否）は、民法423条の債権者代位権という一般原則と登記実務上の取扱いからの推論であり、不動産登記法・同規則に「建物の分割の登記を代位により申請できる」と明示した条文があるわけではありません。また、イの添付情報の根拠として挙げた不動産登記令別表16項の添付情報欄については、ローカル法令データベース収録分での確認にとどまります。公開前に、念のためご自身でも該当条文をご確認いただくことをおすすめします。
- 本文中の「実務でのケース」（エ・オ）の段落は、条文どおりの取扱いを受験生がイメージしやすいように構成した想定事例であり、特定の実在案件・特定の実務教材を出典とするものではありません。登場人物名・依頼内容はすべて説明のための仮設です。
- **再確認・訂正（2026-08-12実施）**：リサーチエージェントによる独立調査で、イの理由づけが事実と逆になっていることが判明し修正しました。訂正前は「通常の分割登記と同様に所有者を証する情報の添付を省略することはできない」としていましたが、正確には通常の分割登記では所有者を証する情報は不要で、共用部分である旨の登記があると表題部所有者の記録・権利に関する登記が抹消される（不動産登記法58条4項）ため、申請人が「所有者」（同法54条2項）であることを別途証明する必要が生じる、という因果関係です。あわせて根拠条文に不動産登記法54条2項を追加しました。正誤判定（誤）自体は変わりません。同内容の修正はブランチmain（コミット717c87e）にも別途適用済みです。
...
Subtitle (smaller, centered, 1行):
平成28年度 午後の部 第13問 作図ガイド（建物の分割の登記）
