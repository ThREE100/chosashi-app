## 【土地家屋調査士受験生向け】平成26年度 第12問〜同時に2件申請、添付情報は使い回せるんです〜

**出題年度：平成26年度　午後の部　第12問**

> 建物の登記の申請情報に関する次のアからオまでの記述のうち，正しいものの組合せは，後記1から5までのうち，どれか。
>
> ア　一棟の建物に属する区分建物の全部について表題登記の申請をする場合には，各別の申請情報によることはできない。
>
> イ　同一の登記所に対して同時に二以上の申請をする場合において，各申請に共通する添付情報を一の申請の申請情報と併せて提供するときは，当該添付情報を当該一の申請の申請情報と併せて提供した旨を他の申請の申請情報の内容としなければならない。
>
> ウ　甲建物の附属建物として登記されている区分建物を分割して，これを乙建物の附属建物に合併しようとする場合において，乙建物の当該附属建物が甲建物の附属建物と接続する区分建物であるときは，建物の分割の登記及び建物の合併の登記を一の申請情報によって申請することはできない。
>
> エ　敷地権付き区分建物の表題登記の申請をする場合において，その敷地権の目的である土地が当該区分建物の所在地を管轄する登記所以外の登記所の管轄区域内にあるときは，当該土地の不動産番号と併せて当該土地を管轄する登記所の表示を申請情報の内容とすれば，当該土地の不動産所在事項，地目及び地積を申請情報の内容とすることを要しない。
>
> オ　区分建物でない建物について区分の登記を申請する場合において，一棟の建物の名称を申請情報の内容とするときは，当該一棟の建物の構造及び床面積を申請情報の内容とすることを要しない。
>
> 1　アイ　　2　アオ　　3　イエ　　4　ウエ　　5　ウオ

建物の登記の申請情報に関するこの問題は、「一括して申請すべきものか」「共通する添付情報をどう扱うか」「省略できる情報は何か」という3つの視点から、細かい手続ルールを問うものです。

### ア：区分建物全部の表題登記は、各別の申請情報によることもできる

一棟の建物に属する区分建物の全部について表題登記の申請をする場合、これらは同時に申請しなければならないという一括申請の原則はありますが、それぞれの区分建物について各別の申請情報（個別の申請書）を作成すること自体は禁止されていません。「各別の申請情報によることはできない」という記述は誤りです。

**たとえば**、1棟のマンションに10戸の区分建物があり、そのすべてについて表題登記を申請する場合、10戸分の申請情報をそれぞれ個別に作成したうえで、これらをまとめて同時に登記所に提出することができます。

### イ：2件以上の同時申請では、共通する添付情報の提供状況を他の申請にも記載する

同一の登記所に対して同時に2件以上の申請をする場合、各申請に共通する添付情報を1つの申請の申請情報とあわせて提供したときは、その旨（当該添付情報をその1つの申請とあわせて提供したこと）を、他の申請の申請情報の内容として記載しなければなりません。添付情報を二重に提出する手間を省くための仕組みです。

**たとえば**、同じ登記所に対して同時に2件の登記を申請する際、両方に共通して必要な書類（住所証明情報など）を1件目の申請書にだけ添付したとします。この場合、2件目の申請書には「その共通の書類は1件目の申請とあわせて提供した」という旨を記載する必要があります。

### ウ：接続する区分建物どうしの分割・合併は、一の申請情報で申請できる

甲建物の附属建物として登記されている区分建物を分割して、これを乙建物の附属建物に合併しようとする場合において、乙建物の当該附属建物が甲建物の附属建物と接続する区分建物であるときは、建物の分割の登記及び建物の合併の登記を一の申請情報によって申請することができます。「一の申請情報によって申請することはできない」という記述は誤りです。

**たとえば**、甲建物の附属建物として登記されている倉庫（区分建物）を切り離し、隣接する乙建物の附属建物としてつなげる場合、この倉庫が乙建物の附属建物と実際に接続しているのであれば、分割の登記と合併の登記を1つの申請書にまとめて申請することができます。

### エ：管轄外の土地が敷地権の目的であるときは、不動産番号と管轄登記所の表示で足りる

敷地権付き区分建物の表題登記を申請する場合において、敷地権の目的である土地が、その区分建物の所在地を管轄する登記所以外の登記所の管轄区域内にあるときは、その土地の不動産番号とあわせてその土地を管轄する登記所の表示を申請情報の内容とすれば、その土地の不動産所在事項・地目・地積を申請情報の内容とすることを要しません。

**たとえば**、A登記所の管轄区域内にある区分建物の敷地権の目的である土地が、実はB登記所の管轄区域内にあったとします。この場合、その土地の不動産番号とB登記所の表示を申請情報に記載すれば、その土地の所在・地目・地積までを重ねて記載する必要はありません。

**ここが分かりにくいポイント**：
「不動産番号を書けば、所在・地目・地積の記載を省略できる」というルール自体は、実は敷地権に限った特別なものではありません。不動産登記令6条は、不動産番号（正確には「不動産識別事項」）を申請情報の内容としたときは、土地・建物の所在事項などの記載を広く省略できると定めています。この原則だけを覚えていると、「じゃあ管轄が違っても、不動産番号さえ書けば省略できるはず」あるいは逆に「管轄が違う土地なんて特殊だから、省略なんてできないはず」と、どちらかの極端な考え方に飛びついてしまいがちです。

正しくは、次の2段階で確認する必要があります。

1. まず、省略しようとしている土地（敷地権の目的である土地）が、申請を受ける登記所（区分建物の所在地を管轄する登記所）自身の管轄区域内にあるか、それとも別の登記所の管轄区域内にあるかを確認します。
2. 同じ登記所の管轄区域内であれば、不動産番号を申請情報の内容とするだけで、所在・地目・地積の記載を省略できます（令6条の原則どおり）。これに対して、別の登記所の管轄区域内にある場合は、不動産番号だけでは足りず、あわせて「その土地を管轄する登記所の表示」も申請情報の内容としなければ、この省略ルールは適用されません（不動産登記規則34条3項）。管轄が違う土地について、登記官が不動産番号だけからその土地を正しく特定できるとは限らないため、追加でその土地の管轄登記所を明示させる仕組みになっているのです。

肢エは、不動産番号と管轄登記所の表示の両方を申請情報の内容とすると述べているので、この2段階の要件をどちらも満たしており、正しい記述です。

なお、敷地権そのものの仕組み（専有部分と敷地利用権をセットで扱う制度）について基礎から確認したい場合は、[敷地権と分離処分可能規約を解説した記事](../topics/shitchiken-bunrishobun-kanou-kiyaku.md)もあわせて参照してください。

### オ：区分建物でない建物を区分の登記する場合、名称があっても構造・床面積の記載は省略できない

区分建物でない建物について区分の登記を申請する場合、一棟の建物の名称を申請情報の内容とするときであっても、当該一棟の建物の構造及び床面積を申請情報の内容とすることを省略することはできません。この場合はまだ一棟の建物としての記録が存在しないため、名称だけでなく構造・床面積も申請情報として必要になります。

**たとえば**、これまで区分建物として登記されていなかった建物を、複数戸に分けて区分建物とする区分の登記を申請するとき、その建物に「〇〇マンション」といった名称を付けて申請情報に記載したとしても、それとは別に建物の構造や床面積についても申請情報に記載する必要があります。

**ここが分かりにくいポイント**：
肢エで「不動産番号を書けば、他の記載事項を省略できる」という省略パターンを読んだ直後にこの肢オを読むと、「今度は名称を書けば、他の記載事項（構造・床面積）を省略できるのだろう」と、同じパターンの繰り返しだと思い込んでしまいがちです。しかし、この2つの肢は見た目が似ているだけで、法律上の扱いはまったく異なります。

正しい判断のステップは、次のとおりです。

1. まず、構造・床面積と名称が、それぞれ何という条文の登記事項なのかを確認します。不動産登記法44条1項は、区分建物又は附属建物である場合の一棟の建物の構造及び床面積を7号で、一棟の建物の名称があるときはその名称を8号で、それぞれ独立した登記事項として定めています。
2. 次に、「一方を書けば他方を省略できる」という規定が、条文のどこかに存在するかを確認します。肢エの不動産番号による省略は、不動産登記令6条という明文の根拠がありました。これに対し、「名称を書けば構造・床面積を省略できる」という規定は、44条にも他の条文にも存在しません。
3. 省略できるのは、あくまで法令が明文で認めた場合に限られます。明文の省略規定がない以上、名称を記載しても、構造・床面積の記載を省略することはできません。

肢エと肢オを対比すると、次のように整理できます。

- **肢エ**：不動産番号→敷地権の目的である土地の所在・地目・地積を省略できる（不動産登記令6条という明文の省略規定がある）
- **肢オ**：名称→一棟の建物の構造・床面積を省略できるわけではない（そのような省略規定が存在しない）

「何かを書けば、何かを省略できる」というパターンそのものを覚えるのではなく、「その省略を認める条文が実際にあるかどうか」を1つずつ確認する姿勢が大切です。

### まとめ

- **ア（誤）**　区分建物全部の表題登記は、各別の申請情報によることもできる（同時申請すればよい）
- **イ（正）**　2件以上の同時申請での共通添付情報の提供状況は、他の申請の申請情報にも記載する
- **ウ（誤）**　接続する区分建物の附属建物どうしの分割・合併は、一の申請情報で申請できる
- **エ（正）**　管轄外の土地が敷地権の目的であるときは、不動産番号と管轄登記所の表示で足りる
- **オ（誤）**　区分建物でない建物を区分の登記する場合、名称があっても構造・床面積の記載は省略できない

「各別の申請情報でよいか」と「同時に申請しなければならないか」は別の論点であることを意識すると、アのような肢に惑わされにくくなります。

**正解：イエの組合せ（選択肢3番）**

---

**このまま使える点／使う前に確認したい点**

- 文章としてはこのままnoteに貼り付けて投稿できる内容です。
- 出題年度・問題番号・正解番号（3番＝イ・エ）は、ユーザー提供の法務省公表試験問題原本および正答資料（PDF）で確認済みです。
- オ（不動産登記法44条1項7号・8号：区分建物又は附属建物が区分建物であるときは、当該一棟の建物の構造及び床面積が登記事項とされ〔7号〕、名称があるときはその名称が別途登記事項とされる〔8号〕こと。名称の存在は7号の構造・床面積の記載を代替・省略する規定にはなっていません）は、`note-articles/laws/fudousan-touki-hou.md`に保存した条文原文で確認できました。イ（不動産登記規則37条1項・2項「同一の登記所に対して同時に二以上の申請をする場合において…当該添付情報を当該一の申請の申請情報と併せて提供した旨を他の申請の申請情報の内容としなければならない」）も条文原文で確認できました。エ（不動産登記法44条・不動産登記令別表：管轄外土地の不動産番号による省略）は条文・規則の内容と整合しています。ア（区分建物一括申請と各別の申請情報の関係）、ウ（接続する区分建物の分割・合併の一括申請可否）についても実務上確立した取扱いとして整理していますが、根拠条文の逐条確認までは至っていない部分があります。
- **重複出題チェック（2026-07-21実施）**：takuitsu.jsonを検索し、平成26年度より後に実施された試験（データベースには平成27年度〜令和7年度が存在）で、本問と同一・類似の問題が再出題されていないかをテキスト類似度と目視確認の両方で確認しました。建物の登記の申請情報の記載事項がテーマ。同一テーマの問題は見当たりませんでした。令和8年度以降の試験がデータベースに追加された際は、この確認を再実施してください。
- **最新法令チェック（2026-08-16再実施）**：`note-articles/laws/`に保存した不動産登記法・不動産登記規則の現行条文（2026-08-04取得）で44条・規則37条を再確認し、肢イの根拠条文を新たに条文上確認しました。直近の法改正で各肢の結論に変更された事実は見当たりません。
- **訂正（2026-08-16）**：肢イの見出し・まとめ・タイトル・見出し画像用フレーズで「3件以上の同時申請」としていましたが、原問題の肢イ・規則37条とも「二以上の申請」（2件から適用）と定めており、「3件以上」は不正確でした。本文（肢イの段落）は当初から「2件以上」と正しく書かれていましたが、見出し等の他の箇所と食い違っていたため、記事全体で「2件以上」に統一しました（3件以上の場合も同様に適用されることは、本文中に維持しています）。あわせてインフォグラフィックCard3の図解も2件の例に修正しました。
- **分かりにくいポイントの追加（2026-09-18）**：エ・オ両肢について、正誤の結論には納得できても、なぜそうなるのか自力でたどり着けなかった、というフィードバックを受け、「ここが分かりにくいポイント」を本文に追加しました。エは、不動産番号による省略（不動産登記令6条）の原則に加え、敷地権の目的である土地が申請先と別の登記所の管轄区域内にある場合は「その土地を管轄する登記所の表示」もあわせて申請情報の内容としない限り省略が適用されない、という不動産登記規則34条3項の追加要件に焦点を当てました。オは、エの「番号を書けば省略できる」というパターンに引きずられて「名称を書けば構造・床面積も省略できる」と誤読しやすい点を、不動産登記法44条1項7号・8号がそれぞれ独立した登記事項であり、名称による省略を認める明文規定が存在しないことから整理しました。あわせて、この2肢を図解する「インフォグラフィック プロンプト（エ肢・間違いノート）」「インフォグラフィック プロンプト（エ・オ対比・間違いノート）」を追加しました。正誤判定・正解の組合せ自体（イエの組合せ、選択肢3番）は変更していません。

---

## 見出し画像用フレーズ

- 区分建物の表題登記、実は個別の申請書でいいんです
- 2件同時申請でも、書類は使い回せるんです
- 接続してれば、分割と合併を一度にできるんです
- 管轄が違う土地でも、番号だけでいいことがあるんです
- 名称があっても、構造・床面積は省略できないんです

---

## インフォグラフィック プロンプト（問題全体）

建物の登記の申請情報について「複数の登記手続を一括してよいか」と「添付情報・記載事項をどこまで省略できるか」という2つの軸に沿って、5肢すべての正しいルールを、通し番号バッジ・短い見出し・図解・短い結論タグだけで構成する2列・5枚のポスター型カードで俯瞰する構成。

```
Create a Japanese-language infographic, portrait layout, 1080x1920 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), rounded card sections, consistent with a
modern explainer-graphic aesthetic (icons: isometric registry desks,
stacked application documents, paperclip/attachment icons, connected
building blocks, unit mailbox rows, registry-office buildings, red cross
marks, green checkmarks, etc. — adapt icon set to the topic).

GLANCEABLE-POSTER REQUIREMENT (critical): This is a quick-reference poster,
NOT a text-heavy explainer document. There is NO intro illustration and NO
paragraph of prose anywhere on this poster — go straight from the header
to the cards. Every card must communicate its point almost entirely
through the illustration (icons, X marks, checkmarks, small embedded
labels) plus one short heading and one short conclusion tag. Do NOT render
any full-sentence explanation, legal citation, or paragraph of body text
anywhere on the poster. If a piece of information cannot be expressed as a
short label (a few words) or drawn as an icon, leave it out rather than
writing it as prose.

CRITICAL TEXT REQUIREMENT: All text must be rendered in standard Japanese
only — hiragana, katakana, and Jōyō (regular Japanese) kanji. Do NOT use
Simplified Chinese characters (simplified hanzi) under any circumstances,
even if a character looks similar. Every kanji must match standard Japanese
orthography exactly as written below, stroke-for-stroke. Reproduce the
exact text strings given below verbatim — do not paraphrase, translate,
summarize, or substitute any characters. Pay special attention to the
kanji 号・地・番・建・物・登・記・所・請・報 — these must be rendered in
their standard Japanese forms, never as Simplified Chinese variants.

--- HEADER ---
Title (large, bold, 1行):
建物の登記、申請情報はどこまで省略できる？

Subtitle (smaller, centered, 1行):
平成26年度　午後の部　第12問－一括申請の可否と添付・記載情報の要否

（タイトル・サブタイトルのすぐ下にカード群を続ける。導入イラスト・導入文の
ブロックは置かない。）

--- COLUMN A HEADER (pill-shaped badge, color: green) ---
一括して申請できるか

--- COLUMN A, CARD 1 ---
Badge: a filled green circle containing the number 1.
Heading (bold, ONE line, ~20 characters or fewer):
区分建物全部でも各別の申請書でOK
Illustration: An isometric apartment building with ten unit icons in a
grid, each unit holding its own small application-document icon
("各別の申請情報"). Arrows from all ten documents converge into one
registry-desk basket labeled "同時提出", with a green checkmark above the
basket.
Conclusion tag (green banner below the illustration, 5-15 characters):
個別書類でも同時提出でOK

--- COLUMN A, CARD 2 ---
Badge: a filled green circle containing the number 2.
Heading (bold, ONE line, ~20 characters or fewer):
接続していれば分割・合併を一括申請
Illustration: Two isometric attached-building blocks, one labeled "甲建物
の附属建物", the other labeled "乙建物の附属建物", joined together along a
shared wall with a bridge icon showing "接続". A single application
document above them is labeled "分割の登記＋合併の登記" with a green
checkmark.
Conclusion tag (green banner below the illustration, 5-15 characters):
一の申請情報でOK

--- COLUMN B HEADER (pill-shaped badge, color: blue) ---
添付・記載情報は省略できるか

--- COLUMN B, CARD 3 ---
Badge: a filled blue circle containing the number 3.
Heading (bold, ONE line, ~20 characters or fewer):
共通の添付書類は他の申請に一言添える
Illustration: Two isometric application-document stacks side by side at
a registry desk, labeled "①" and "②", with a small note "2件以上で適用"
above them. Only the first document has a paperclip icon attached,
labeled "住所証明情報". The second document shows a note tag reading
"①の添付情報を援用" instead of a duplicate paperclip.
Conclusion tag (blue banner below the illustration, 5-15 characters):
他の申請にも記載

--- COLUMN B, CARD 4 ---
Badge: a filled blue circle containing the number 4.
Heading (bold, ONE line, ~20 characters or fewer):
管轄外の土地は番号＋登記所表示でOK
Illustration: Two isometric registry-office buildings labeled "A登記所"
and "B登記所" connected by a dashed line crossing a prefecture border. A
document panel shows tags "不動産番号" and "B登記所の表示" checked green,
while tags "所在" "地目" "地積" have red cross marks over them.
Conclusion tag (blue banner below the illustration, 5-15 characters):
所在・地目・地積は省略可

--- COLUMN B, CARD 5 ---
Badge: a filled blue circle containing the number 5.
Heading (bold, ONE line, ~20 characters or fewer):
名称があっても構造・床面積は必須
Illustration: An isometric building with a nameplate reading
"〇〇マンション" and a green checkmark beside it. Next to the nameplate, two
small document icons labeled "構造" and "床面積" are shown with a red cross
mark over an attempt to remove them, indicating they cannot be omitted.
Conclusion tag (blue banner below the illustration, 5-15 characters):
構造・床面積は省略不可

--- FOOTER ---

Final check before rendering: scan every kanji glyph and confirm it is
standard Japanese (Jōyō) form, not Simplified Chinese. If any character
renders as a Simplified Chinese variant, redraw that character in the
correct Japanese form. Confirm the number of cards equals 5 exactly, with
no duplicated or missing cards, that badge numbers run 1-5 continuously
across both columns without resetting, confirm there is no intro
illustration or paragraph block between the header and the cards, and
confirm that no card contains a full sentence of explanatory prose —
every card's takeaway must read as a short heading + a short conclusion
tag, at a glance.
```

---

## インフォグラフィック プロンプト（エ肢・間違いノート）

肢エでつまずきやすいのは、「不動産番号があれば省略できる」という原則だけで判断を止めてしまい、管轄が違う場合に追加で必要になる要件（登記所の表示）を見落としてしまう点です。2段階の条件判定を決定木で示します。

```
Create a Japanese-language infographic, portrait layout, 1080x1920 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), rounded panel sections, consistent with the
same visual language as the whole-problem poster for this article, but
built as a single detailed explainer panel rather than a multi-card
poster.

MISTAKE-NOTEBOOK EXPLAINER REQUIREMENT: This image exists to resolve one
specific point of confusion, not to be a glanceable summary. Unlike a
quick-reference poster, this image MAY include flowchart branches with
written conditions, side-by-side comparisons with quoted article text,
and short explanatory sentences (1-3 sentences per callout box) labeled
「誤りやすいポイント」or similar. Prioritize clarity and completeness of
the reasoning over brevity.

CRITICAL TEXT REQUIREMENT: All text must be rendered in standard Japanese
only — hiragana, katakana, and Jōyō (regular Japanese) kanji. Do NOT use
Simplified Chinese characters (simplified hanzi) under any circumstances,
even if a character looks similar. Reproduce the exact text strings given
below verbatim — do not paraphrase, translate, summarize, or substitute
any characters. Pay special attention to the kanji 敷・地・権・番・号・
管・轄・登・記・所・省・略・誤 — always draw the standard Japanese (Jōyō)
form.

BACKGROUND REQUIREMENT (critical): The entire canvas must be fully opaque
from edge to edge. Do NOT generate a transparent or alpha-channel
background under any circumstances, even if the output file format
supports transparency. Fill the full canvas — including every corner and
margin outside the panel/flowchart/comparison — with a solid or
illustrated opaque background. There must be no checkerboard pattern, no
partially transparent area, and no unpainted canvas edge anywhere in the
final image.

--- HEADER ---
Title (large, bold):
「番号だけ」で足りるとは限らない

Subtitle (smaller, centered):
平成26年度 午後の部 第12問 エ－敷地権の目的土地、管轄が違うときの追加要件

--- FLOWCHART ---
Start node (rectangle, isometric document icon):
敷地権付き区分建物の表題登記を申請する（敷地権の目的である土地の不動産番
号を申請情報に記載する）

Down arrow to Decision node 1 (diamond icon):
その土地は、申請を受ける登記所（区分建物の所在地を管轄する登記所）自身の
管轄区域内にあるか？

Branch left（あり、green checkmark）:
Result node: 不動産番号を記載するだけで、所在・地目・地積の記載を省略でき
る（不動産登記令6条の原則どおり）

Branch right（なし＝別の登記所の管轄区域内、続けて次の判定へ）leading to
Decision node 2 (diamond icon):
不動産番号とあわせて、その土地を管轄する登記所の表示も申請情報の内容とし
たか？

From Decision node 2, Branch left（していない、red X icon）:
Result node: 省略ルールは適用されない → 所在・地目・地積の記載が必要にな
る

From Decision node 2, Branch right（している、green checkmark、highlighted
as the case in 肢エ）:
Result node: 不動産登記規則34条3項の要件を満たす → 所在・地目・地積の記載
を省略できる

--- CALLOUT: 誤りやすいポイント ---
「不動産番号があれば省略できる」という原則だけを覚えていると、管轄が違う
土地について「番号だけで足りるはず」と考えたり、逆に「管轄が違うなら省略
できないはず」と考えたりしがちです。実際には、管轄が違う場合に限って
「その土地を管轄する登記所の表示」もあわせて記載することが、省略の追加
条件になります（不動産登記規則34条3項）。

--- FOOTER ---
根拠条文：不動産登記令6条1項3号、不動産登記規則34条2項・3項。判例・先例
の具体的な番号はここには書かない。

Final check before rendering: scan every kanji glyph and confirm it is
standard Japanese (Jōyō) form, not Simplified Chinese. Confirm every
heading, node label, and callout text matches the Japanese text given
above verbatim, with no paraphrasing and no substituted characters,
confirm the flowchart has exactly 2 sequential decision nodes (not merged
into one), confirm nothing is rendered below the FOOTER's small footnote
text (no summary recap panel, no trophy or medal icon, no re-listed ○/✕
grid, and no additional text block of any kind), and confirm the entire
canvas, edge to edge, is filled with a fully opaque background with no
transparency or alpha channel anywhere.
```

---

## インフォグラフィック プロンプト（エ・オ対比・間違いノート）

エとオはどちらも「何かを記載すれば、何かを省略できるか」という形の肢ですが、結論が正反対です。エは明文の省略規定があるため省略できる例、オは明文の省略規定が存在しないため省略できない例として、左右対比で示します。

```
Create a Japanese-language infographic, portrait layout, 1350x1500 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), a rounded panel containing a clean two-column
comparison, consistent with the same visual language as the whole-problem
poster for this article, built as a single detailed explainer panel.

MISTAKE-NOTEBOOK EXPLAINER REQUIREMENT: This image exists to resolve one
specific point of confusion (confusing two similar-looking 肢 with opposite
conclusions), not to be a glanceable summary. Unlike a quick-reference
poster, this image MAY include side-by-side comparisons with quoted
article text and short explanatory sentences (1-3 sentences per callout
box) labeled「誤りやすいポイント」or similar. Prioritize clarity and
completeness of the reasoning over brevity.

CRITICAL TEXT REQUIREMENT: All text must be rendered in standard Japanese
only — hiragana, katakana, and Jōyō (regular Japanese) kanji. Do NOT use
Simplified Chinese characters (simplified hanzi) under any circumstances,
even if a character looks similar. Reproduce the exact text strings given
below verbatim — do not paraphrase, translate, summarize, or substitute
any characters. Pay special attention to the kanji 番・号・省・略・構・造・
床・面・積・名・称・条・誤 — always draw the standard Japanese (Jōyō) form.

BACKGROUND REQUIREMENT (critical): The entire canvas must be fully opaque
from edge to edge. Do NOT generate a transparent or alpha-channel
background under any circumstances, even if the output file format
supports transparency. Fill the full canvas — including every corner and
margin outside the comparison panel — with a solid or illustrated opaque
background. There must be no checkerboard pattern, no partially
transparent area, and no unpainted canvas edge anywhere in the final
image.

--- HEADER ---
Title (large, bold):
省略できる／できないは、条文の有無で決まる

Subtitle (smaller, centered):
平成26年度 午後の部 第12問　エ vs オ－似ているようで結論が逆の2肢

--- COMPARISON ---
LEFT PANEL (green, heading「肢エ：省略できる」):
書くもの：不動産番号
省略できるもの：敷地権の目的である土地の所在・地目・地積
根拠：不動産登記令6条1項3号という明文の省略規定がある
結論：正しい記述（省略できる）

RIGHT PANEL (blue, heading「肢オ：省略できない」):
書くもの：一棟の建物の名称
省略を狙うもの：一棟の建物の構造及び床面積
根拠：不動産登記法44条1項7号（構造・床面積）と8号（名称）は独立した別の
登記事項であり、名称による省略を認める規定は存在しない
結論：誤った記述（省略できない）

--- CALLOUT: 誤りやすいポイント ---
2つの肢は「何かを書けば、何かを省略できるか」という同じ形をしているた
め、エを先に読むと、オも同じパターンの繰り返しだろうと考えてしまいがち
です。しかし省略が認められるのは、あくまで条文が明文で認めた場合に限ら
れます。「パターンが似ている」ことと「省略が認められる」ことは別問題で
す。

--- FOOTER ---
根拠条文：不動産登記令6条1項3号、不動産登記法44条1項7号・8号。判例・先例
の具体的な番号はここには書かない。

Final check before rendering: scan every kanji glyph and confirm it is
standard Japanese (Jōyō) form, not Simplified Chinese. Confirm every
heading, panel label, and callout text matches the Japanese text given
above verbatim, with no paraphrasing and no substituted characters,
confirm the LEFT and RIGHT panels are aligned at the same height item by
item (書くもの／省略できるもの・省略を狙うもの／根拠／結論), confirm
nothing is rendered below the FOOTER's small footnote text (no summary
recap panel, no trophy or medal icon, no re-listed ○/✕ grid, and no
additional text block of any kind), and confirm the entire canvas, edge to
edge, is filled with a fully opaque background with no transparency or
alpha channel anywhere.
```

---

## インフォグラフィック プロンプト（ア〜オ 作図ガイド）

問題文を読んだ瞬間に「どの図を描き、どの順番で条件を確認すればよいか」を、ア〜オ5肢それぞれについて示す作図ガイド。②の結論カードポスターとは別物で、②の内容は書き換えない。肢エは管轄区域の内外→登記所表示の有無という2段階の決定木、肢アは「同時申請の要否」と「各別申請書の可否」という別々の論点を切り分ける対比枠、肢オは肢エのパターンに引きずられやすい思い込みとの正誤対比で構成する。

```
Create a Japanese-language infographic, portrait layout, 1080x2600 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), rounded panel sections, consistent with the
same visual language as the whole-problem poster for this article, but
built as a set of 5 diagram-drawing panels (a "how to sketch this fact
pattern, in the right order" study reference) rather than a
quick-reference conclusion poster.

DIAGRAM-GUIDE REQUIREMENT (critical): Each panel's purpose is to show the
reader exactly what diagram they should draw on scratch paper while
reading this type of problem, AND the order in which they should check
conditions to get there — isometric registry desks, stacked application
documents, paperclip/attachment icons, connected building blocks showing
the 分割 (division) and 合併 (merger) of 附属建物 between 甲建物 and 乙
建物, apartment/condominium unit grids for 区分建物, prefecture-border
registry-office buildings, green checkmarks, and red cross marks. Where a 肢 requires checking multiple
conditions in sequence before reaching a conclusion, draw the panel's
diagram as an actual decision flowchart: diamond-shaped branch nodes with
the condition written on them, Yes/No (or ○/✕) branch arrows, and a final
conclusion node. Where a 肢 is resolved by a single check, a labeled
illustrative diagram is sufficient — do not force a flowchart. Where a
panel reuses a decision tree shared with other panels in this set, render
the branch relevant to THIS panel with a thick highlighted border and full
color, and render the other, unrelated branches in a faded, greyed-out, or
dotted-outline style rather than omitting them — the reader should be able
to see at a glance which part of the shared tree this panel is about.
Likewise, where a diagram must show an entity that conceptually never held
the right or status in question, render that entity in a faded or
dotted-outline style rather than a plain ○/✕ mark, so the visual itself
communicates "this was never really there," not just "this is wrong."
Unlike a glanceable summary poster, each panel MAY include a short「着眼
点」callout box with 1-2 sentences that state the checking ORDER in words
(e.g. "まず〜を確認し、次に〜を確認します"), not just the conclusion. Do
not include case or precedent numbers (article/regulation numbers are
fine); keep the callout text as written below verbatim, and keep every
condition each callout describes faithful to the article's own body
text — do not drop or merge a required element.

CRITICAL TEXT REQUIREMENT: All text must be rendered in standard Japanese
only — hiragana, katakana, and Jōyō (regular Japanese) kanji. Do NOT use
Simplified Chinese characters (simplified hanzi) under any circumstances,
even if a character looks similar. Do NOT use Traditional Chinese
characters (traditional hanzi) either, even where a traditional-hanzi
glyph looks close to the correct Japanese kanji form — every glyph must
match the standard Japanese Jōyō form exactly, not the Chinese
traditional variant. Do NOT render any character that is not standard
Japanese hiragana, katakana, or Jōyō kanji anywhere in the image —
no Chinese-only characters, no Korean Hangul, no other non-Japanese
script, and no stray or decorative glyphs of any kind, even as small
background or texture elements. Reproduce the exact text strings given
below verbatim — do not paraphrase, translate, summarize, or substitute
any characters. Within this English prompt text, use half-width
parentheses ( ) consistently — never open a parenthetical with a
full-width （ and close it with a half-width ), or vice versa.

BACKGROUND REQUIREMENT (critical): The entire canvas must be fully opaque
from edge to edge. Do NOT generate a transparent or alpha-channel
background under any circumstances, even if the output file format
supports transparency. Fill the full canvas — including every corner and
margin outside the panels — with a solid or illustrated opaque background
(the pale beige/gray tone used elsewhere in this style is a good
default). There must be no checkerboard pattern, no partially transparent
area, and no unpainted canvas edge anywhere in the final image.

--- HEADER ---
Title (large, bold, 2行):
問題文を読んだら
どんな図を描けばいいか

Subtitle (smaller, centered, 1行):
平成26年度 午後 第12問 作図ガイド（建物登記の申請情報）

（タイトル・サブタイトルのすぐ下にパネル群を続ける。導入イラスト・導入文の
ブロックは置かない。）

--- PANEL 1（肢ア） ---
Badge: a filled circle in blue containing the number 1 (numbers run
continuously through all panels).
Heading (bold, ONE line):
同時申請の要否と各別申請書の可否を分けて考える
Diagram: Two side-by-side frames rather than a flowchart (this 肢 turns on
telling two different questions apart, not on a sequence of conditions).
LEFT FRAME, labeled「論点1：同時に申請しなければならないか」: an isometric
apartment building showing a grid of ten separate 区分建物 units, each
holding its own small application-document icon, all ten documents
flowing toward one registry-desk basket at the same moment, with a note
「原則として同時に申請」. RIGHT FRAME, labeled「論点2：各別の申請情報でよ
いか」: the same ten documents shown as ten clearly separate,
individually bound stacks (not merged into one document), with a green
checkmark and label「各別の申請情報でOK」. Between the two frames, a
small connecting icon shows a crossed-out equals sign with the note「こ
の2つは別の論点」, visually separating「同時申請」from「1通にまとめる必
要」.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、10戸分をまとめて同時に登記所へ提出する必要があるか（論点1）を確認
します。次に、その提出物が1通の申請情報にまとまっていなければならないか
（論点2）を分けて確認すると、各別の申請情報でも差し支えないことが分かり
ます。
Conclusion tag (a short colored banner/pill, blue, 5-15 Japanese
characters):
各別の申請書でもOK

--- PANEL 2（肢イ） ---
Badge: a filled circle in green containing the number 2 (numbers run
continuously through all panels).
Heading (bold, ONE line):
共通添付情報は他の申請にもその旨を記載する
Diagram: A single-check illustrative diagram (not a flowchart). An
isometric registry desk with two application-document stacks side by
side, labeled「①」and「②」, with a small note「2件以上の同時申請」above
them. Only stack ① has a paperclip icon attached, labeled「住所証明情
報」. Stack ② has no duplicate paperclip; instead it carries a note tag
reading「①の添付情報を援用」, together with a green checkmark.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、同一の登記所に2件以上の申請を同時に行っているかを確認します。次
に、共通する添付情報を1件の申請にだけ添付した場合は、残りの申請の申請情
報にも「その添付情報を援用した」旨を書き添える必要があることを確認しま
す。
Conclusion tag (a short colored banner/pill, green, 5-15 Japanese
characters):
他の申請にも記載

--- PANEL 3（肢ウ） ---
Badge: a filled circle in blue containing the number 3 (numbers run
continuously through all panels).
Heading (bold, ONE line):
接続する附属建物どうしは一の申請情報でまとめる
Diagram: A single-check illustrative diagram (not a flowchart; this 肢's
own wording already assumes a scene where the single fact "接続する区分
建物であるとき" holds true). Two isometric attached-building blocks: the
附属建物 (区分建物) of 甲建物 is shown being separated by a division
arrow (分割), then joined by a merger arrow (合併) to the 附属建物 of 乙
建物. Between the two, a bridge/shared-wall icon is placed and labeled
「接続」. Above them, a single combined application document labeled
「分割の登記＋合併の登記」is shown with a green checkmark, beside which a
red cross mark is placed over an attempt to forcibly split this into two
separate application documents.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、分割後の附属建物（区分建物）が、合併先である乙建物の附属建物と実際
に接続しているかを確認します。接続していれば、分割の登記と合併の登記をま
とめて一の申請情報で申請できると確認できます。
Conclusion tag (a short colored banner/pill, blue, 5-15 Japanese
characters):
一の申請情報でOK

--- PANEL 4（肢エ） ---
Badge: a filled circle in green containing the number 4 (numbers run
continuously through all panels).
Heading (bold, ONE line):
管轄の内外を確認する
Diagram: An actual decision flowchart with two sequential diamond-shaped
branch nodes (both branches of both nodes matter and must be shown; do
not draw any arrow that loops back to an earlier node).
Start node (rectangle, isometric document icon) labeled "敷地権付き区分建
物の表題登記を申請する（敷地権の目的である土地の不動産番号を申請情報に
記載する）". An arrow leads down to Decision node 1 (diamond icon)
labeled "その土地は、申請を受ける登記所（区分建物の所在地を管轄する登記
所）自身の管轄区域内にあるか？". The Yes branch (green checkmark) leads
to a Result node labeled "不動産番号を記載するだけで、所在・地目・地積の
記載を省略できる". The No branch (meaning the land is within a different
registry office's jurisdiction) leads to Decision node 2 (diamond icon)
labeled "不動産番号とあわせて、その土地を管轄する登記所の表示も申請情報
の内容としたか？". From Decision node 2, the No branch (red X icon)
leads to a Result node labeled "省略ルールは適用されず、所在・地目・地積
の記載が必要になる". From Decision node 2, the Yes branch (green
checkmark, highlighted with a thick border as the case this 肢 assumes)
leads to a Result node labeled "所在・地目・地積の記載を省略できる".
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、敷地権の目的である土地が申請先の登記所自身の管轄区域内にあるかを確
認します。管轄区域外であれば、次に、その土地を管轄する登記所の表示もあわ
せて記載したかを確認し、両方を記載していれば所在・地目・地積の記載を省略
できます。
Conclusion tag (a short colored banner/pill, green, 5-15 Japanese
characters):
登記所表示があれば省略可

--- PANEL 5（肢オ） ---
Badge: a filled circle in blue containing the number 5 (numbers run
continuously through all panels).
Heading (bold, ONE line):
名称を書いても構造・床面積は省略できない
Diagram: A 正誤対比 (right-rule-vs-common-mistake) style comparison, not
a flowchart. LEFT SIDE, labeled「誤りやすい思い込み」, faded/greyed
illustration: reusing the same arrow icon from 肢エ's "不動産番号を書け
ば省略できる" idea, with a red cross mark placed over an arrow labeled
「名称の記載 → 構造・床面積の省略」. RIGHT SIDE, labeled「正しいルー
ル」, highlighted in full color: beneath a speech bubble quoting 不動産
登記法44条1項, two registration-item tags reading「7号：構造・床面積」
and「8号：名称」are shown, each with a green checkmark, placed clearly
apart with no connecting arrow drawn between them, visualizing that
there is no rule under which recording one lets the other be omitted.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、構造・床面積と名称が、それぞれ不動産登記法44条1項の何号の登記事項
かを確認します。次に、一方を記載すれば他方を省略できるという規定が条文
上あるかを確認すると、そのような規定は存在せず、名称を記載しても構造・
床面積の記載は省略できないと分かります。
Conclusion tag (a short colored banner/pill, blue, 5-15 Japanese
characters):
名称では省略不可

--- FOOTER ---
Small footnote text (bottom of panel, small font, verbatim):
根拠条文：不動産登記令6条1項3号、不動産登記規則34条2項・3項・37条1項・2
項、不動産登記法44条1項7号・8号。判例・先例の具体的な番号はここには書か
ない。

Final check before rendering: scan every kanji glyph and confirm it is
standard Japanese (Jōyō) form, not Simplified Chinese and not Traditional
Chinese, paying special attention to 同・各・別・添・旨・接・続・敷・轄・
略・構・床・積・称. If any character renders as a Simplified or
Traditional Chinese variant, redraw that character in the correct
Japanese form. Also scan the entire canvas for any character that is not
standard Japanese hiragana, katakana, or Jōyō kanji — including any
Chinese-only character, Korean Hangul, other non-Japanese script, or
stray decorative glyph — and remove or redraw it so that only standard
Japanese text appears anywhere in the image. Confirm the panel count
equals 5 exactly, badge numbers run 1-5 continuously, there is no intro
illustration or paragraph block between the header and the panels, that
every multi-condition 肢 (panel 4) is drawn as an actual flowchart with
branch nodes (not a bare illustration with no visible decision
structure), that panel 4's second decision node shows both the「してい
る」and「していない」branches with their own distinct result nodes (no
arrow loops back to an earlier node), that each 着眼点 callout states a
checking order rather than only a conclusion and keeps every required
element from the source article distinct (no merged or dropped
requirements), confirm nothing is rendered below the last panel's
footnote text (no summary recap panel, no trophy or medal icon, no
re-listed ○/✕ grid of all 肢, and no additional text block of any kind),
and confirm the entire canvas, edge to edge, is filled with a fully
opaque background with no transparency or alpha channel anywhere.
```
