...
- **エ（誤）**　附属建物と他の建物の合体では、分割の登記と合体による登記等は別々の申請情報で申請しなければならない（一括申請はできない）
...
の登記＋合体による登記等）" has a large red X over the entire box, while
two separate application-document icons labeled "分割の登記" and "合体
による登記等" each sit apart with their own green checkmark, showing they
...
first (labeled "①分割の登記"), then merging into 乙building (labeled "②
合体による登記等"). Two separate application-document icons are shown
side by side, each with its own green checkmark, with a large red X over
a bundled box behind them labeled "一の申請情報にまとめる" — showing
bundling into one filing is not possible; ①と② must always be submitted
...
Heading (bold, ONE line, ~20 characters or fewer):
合併は記録の組替え、合体は新規登記
Illustration: A split-panel comparison. Left panel: an isometric building
already fully registered (labeled "登記記録あり") sliding sideways into a
folder labeled "他の建物の附属建物として登記記録上を組替え" — no new
document is created, just an arrow moving the existing record, labeled
"合併の登記". Right panel: two isometric buildings physically merging via
construction (labeled "物理的に合体"), with their two old registration
folders being crossed out ("抹消") while a brand-new blank document is
being filled in from scratch, labeled "新しい表題登記". A small connecting
note between the panels reads "附属建物はまず分割で独立した建物になって
から", pointing from the 分割 concept toward the right panel.
Conclusion tag (green banner below the illustration, 5-15 characters):
合体は性質が違う
...
Subtitle (smaller, centered, 1行):
平成26年度　午後の部　第17問 作図ガイド（合体による登記等の手続）
