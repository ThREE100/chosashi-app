...
| 肢 | 判定 | ポイント |
|---|---|---|
| ア | 誤 | 添付図面の訂正の申出は任意であり、「しなければならない」義務ではない（不登規88条1項） |
| イ | 正 | 用紙が数枚にわたるときは、余白に総枚数と何枚目かを記載する（準則51条5項、建物図面・各階平面図に準用） |
| ウ | 正 | 電子署名して提供する図面にも、作成年月日・申請人・作成者名を記録する（不登規73条） |
| エ | 誤 | 建物図面の縮尺は原則500分の1で、法14条地図と同一である必要はない（不登規82条） |
| オ | 誤 | 隔壁除去で物理的に変化したときは、訂正の申出ではなく変更の手続による（不登規88条1項ただし書） |
...
- 各肢の根拠のうち、肢イ（用紙複数枚のときの総枚数・枚数の記載）と肢エ（建物図面の縮尺は原則500分の1）は、内容として確認できたものです。当初は不動産登記規則・準則の該当条文番号（規則51条／73条／74条／82条／88条・準則51条5項などの候補）を一つに断定できていませんでしたが、2026年8月の追加調査（下記参照）でそれぞれの条番号を確定し、本文・まとめ表に反映しました。
...
箇所に総枚数と何枚目かを記載する(準則51条5項、建物図面・各階平面図に
準用)。
...
年月日並びに申請人及び作成者の氏名又は名称を記録しなければならない(不
登規73条)。
...
match.
Caption (small text below):
建物図面は原則として500分の1で作成し(不登規82条)、敷地に法14条1項の地
図が備え付けられていても、その地図と同一の縮尺にする必要はない。
...
の申出は所有者などが任意にできる手続であり、義務ではない(不登規88条1
項)。
...
め、訂正の申出ではなく変更の登記に対応する手続による(不登規88条1項た
だし書)。
