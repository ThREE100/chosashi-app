...
7. 記事ファイルの末尾（既存の②インフォグラフィックプロンプトの後）に `## インフォグラフィック プロンプト（1〜5 作図ガイド）` の見出しを立てて追記する（肢の数が5でない場合は`1〜{肢の数}`とする。例：10肢の問題なら`1〜10`）。既存の②のセクションは変更しない。**「ア〜オ」のような文字（肢の記号）による見出し・Subtitle・パネル表記は禁止（重要・厳守、2026-09-25追加）**：問題文の引用や本文解説では引き続き「ア」「イ」…の記号を使ってよいが、⑤のインフォグラフィックプロンプト側（見出し・Subtitle・`--- PANEL {N}（肢{番号}） ---`の表記）は、必ず肢の記号ではなく通し番号（1・2・3…）で統一する。既存記事に「ア〜オ」表記が残っている場合は、発見しだいこの数字表記に修正する。
