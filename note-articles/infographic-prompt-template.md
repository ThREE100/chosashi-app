...
`bunya-kaisetsu/`（分野別・総合記事）および `topics/`（個別テーマ記事。地目23種類、筆界特定制度、表示登記の添付情報早見表 等）を対象に、記事の内容を土地家屋調査士試験の初学者向けに視覚化する画像生成プロンプトを作成するルール。②（問題別）と違い、**1記事＝1枚に縛られない**。記事の構成（見出し・早見表・対比表）に応じて自然に分かれる単位ごとに、必要な枚数だけプロンプトを作る。
...
1. 対象記事（`note-articles/topics/{slug}.md` または `note-articles/bunya-kaisetsu/{slug}.md`）を読み込む。
