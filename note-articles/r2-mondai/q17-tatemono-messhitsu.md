## 【土地家屋調査士受験生向け】令和2年度 第17問・建物が無くなったら？滅失の登記のルール〜申請人・仮登記名義人・相続・一棟の滅失〜

**出題年度：令和2年度　午後の部　第17問**

> 建物の滅失の登記に関する次のアからオまでの記述のうち，誤っているものの組合せは，後記1から5までのうち，どれか。
>
> ア　敷地権の登記がされた後に抵当権の設定の登記がされた区分建物について滅失の登記を申請する場合において，申請情報と併せて，当該抵当権の登記名義人が敷地権の目的である土地について抵当権を消滅させることを承諾したことを証する情報が提供されたときは，建物の登記記録に，その土地について抵当権が消滅した旨が記録される。
>
> イ　建物の滅失の登記の申請情報及びその添付情報は，受付の日から30年間保存される。
>
> ウ　焼失した建物に所有権の移転の仮登記がされている場合において，当該仮登記の登記名義人は，消防署の焼失の証明書及び所有権の登記名義人の承諾を証する情報を提供すれば，当該建物の滅失の登記の申請をすることができる。
>
> エ　建物の所有権の登記名義人が死亡した後に当該建物が滅失した場合，その相続人は，相続による所有権の移転の登記を行った後でなければ，当該建物の滅失の登記を申請することができない。
>
> オ　所有者が異なる数個の区分建物が属する一棟の建物が滅失した場合において，一棟の建物の滅失の登記の申請は，区分建物の所有者の一人からすることができる。
>
> 1　アイ　　2　アエ　　3　イオ　　4　ウエ　　5　ウオ

建物の滅失の登記は、取壊しや焼失などで建物がなくなったときにする登記です。誰が申請できるのか（申請人の範囲）、相続や仮登記がからむとどうなるのか、といった論点が問われます。今回は**誤っているもの**の組合せを選びます。

### ア：抵当権消滅の承諾情報を出せば、土地について抵当権が消滅した旨が記録される

敷地権の登記がされた後に抵当権が設定された区分建物（＝いわゆる特定登記がある区分建物）について滅失の登記を申請する際、抵当権の登記名義人が敷地権の目的である土地について抵当権を消滅させることを承諾したことを証する情報が提供されたときは、その承諾に係る土地について、当該権利が消滅した旨が記録されます（不動産登記法55条4項）。本肢は正しい記述です。

**たとえば**、マンションの一室（敷地権付き）を取り壊すとき、その部屋についていた抵当権を土地についても消してよいという抵当権者の承諾書が出されれば、土地の分の抵当権が消えた旨が登記記録に記録されます。

### イ：滅失登記の申請情報・添付情報は、受付の日から30年間保存される

表示に関する登記の申請情報およびその添付情報は、受付の日から30年間保存されます（不動産登記規則28条9号）。建物の滅失の登記もこれに含まれます。本肢は正しい記述です。

**たとえば**、建物を取り壊して滅失の登記をしたときの申請書や添付書類は、登記所で30年間は保存されます。すぐに廃棄されるわけではありません。

### ウ：所有権移転の仮登記名義人は、滅失の登記を申請できない

建物が滅失したときに滅失の登記を申請しなければならないのは、表題部所有者または所有権の登記名義人です（不動産登記法57条）。所有権移転の仮登記の登記名義人は、まだ確定的な所有権を取得しておらず、滅失登記の申請人にはなれません。消防署の焼失証明書や所有権の登記名義人の承諾を得ても、仮登記名義人が自ら滅失の登記を申請することはできません。本肢は「申請をすることができる」としており、誤りです。

**たとえば**、Ａ名義の建物についてＢが所有権移転の仮登記をしていた場合、その建物が焼失しても、仮登記名義人Ｂが「焼失証明書とＡの承諾書があるから」といって自分で滅失の登記を申請することはできません。申請するのは所有権の登記名義人であるＡ（またはその相続人）です。

### エ：相続人は、相続登記を経なくても滅失の登記を申請できる

建物の所有権の登記名義人が死亡した後にその建物が滅失した場合、相続人は、相続による所有権移転の登記を経ることなく、自分が相続人であることを証する情報を提供して、滅失の登記を申請できます（不動産登記法30条）。表示に関する登記は、一般承継人が被相続人に代わって申請できるからです。「相続登記を行った後でなければ申請することができない」とする本肢は誤りです。

**たとえば**、父名義の建物が父の死後に取り壊された場合、子は、まず父から自分への相続登記をしなくても、戸籍など相続を証する情報を添えて、その建物の滅失の登記を申請できます。

### オ：一棟の建物の滅失の登記は、区分建物の所有者の一人から申請できる

所有者が異なる数個の区分建物が属する一棟の建物が滅失した場合、一棟の建物の滅失の登記の申請は、区分建物の所有者の一人からすることができます。本肢は正しい記述です。

**たとえば**、複数の所有者がいるマンション（一棟の建物）が全部取り壊されたとき、その一棟の建物の滅失の登記は、区分所有者全員がそろわなくても、そのうちの一人が申請できます。

### まとめ

- **ア（正）**　抵当権消滅の承諾情報を出せば、土地について抵当権が消滅した旨が記録される（不動産登記法55条4項）
- **イ（正）**　滅失登記の申請情報・添付情報は受付の日から30年間保存（不動産登記規則28条9号）
- **ウ（誤）**　所有権移転の仮登記名義人は滅失の登記の申請人になれない（不動産登記法57条）
- **エ（誤）**　相続人は相続登記を経ずに滅失の登記を申請できる（不動産登記法30条）
- **オ（正）**　一棟の建物の滅失の登記は区分建物の所有者の一人から申請できる

「表示登記の申請人になれるのは誰か」「相続人は相続登記なしで表示登記を申請できる」という表示登記共通のルールが、本問を解くカギです。

**正解：ウエの組合せ（選択肢4番）**

---

**このまま使える点／使う前に確認したい点**

- 文章としてはこのままnoteに貼り付けて投稿できる内容です。
- 出典（令和2年度午後の部 第17問）・正解番号（ウエ＝4番）は、土地家屋調査士試験対策アプリ制作時に検証済みのデータベース（takuitsu.json）で確認済みです。
- 各肢の根拠のうち、ア（不動産登記法55条4項）・イ（不動産登記規則28条9号）・ウ（不動産登記法57条＝申請人の範囲）・エ（不動産登記法30条＝一般承継人による申請）は、データベースのexplanationフィールドに条文番号まで明記されています。オ（一棟の建物の滅失の登記を区分建物の所有者の一人から申請できること）は、区分建物の滅失登記の一般的な取扱いからの説明です。
- なお、ローカルのアガルート教材フォルダも確認しましたが、択一式については講義動画のチャプター一覧（対照表PDF）のみが保存されており、テキスト解説そのものは含まれていなかったため、今回は参照できませんでした。
- **重複出題チェック（2026-07-21実施）**：takuitsu.jsonを検索し、令和2年度より後に実施された試験（令和3〜7年度）の全問題を確認しました。**本問オ（所有者が異なる区分建物が属する一棟の建物の滅失登記は区分建物の所有者の一人から申請できる＝正しい）は、令和6年度第17問ウとほぼ同一の文言・同一の結論（正しい）で再出題されています**。また本問エ（相続人は相続登記を経た後でなければ滅失登記を申請できない＝誤り）も、令和4年度第16問ア・令和6年度第17問イと同種の論点（相続人による表示登記の申請に相続登記は不要）を扱っていますが、こちらは具体的な文言が大きく異なります。他の肢（抵当権消滅の記録・保存期間・仮登記名義人の申請適格）は令和3〜7年度の問題には見当たらず、出題全体としては別の問題ですが、上記オの1肢についてはnoteでの執筆・公開に際して「たとえば」の具体例が似た内容にならないよう注意してください。令和8年度以降の試験がデータベースに追加された際は、この確認を再実施してください。

---

## 見出し画像用フレーズ

- 抵当権消滅の承諾があれば、土地の分も消えた旨が記録されるんです
- 滅失登記の書類、30年間は保存されるって知ってた？
- 仮登記名義人は、滅失の登記を申請できないんです
- 相続人は、相続登記なしで滅失の登記を申請できるんです
- 一棟の建物の滅失、区分所有者の一人からできるんです

---

## インフォグラフィック プロンプト（問題全体）

建物の滅失の登記について、「誰が申請人になれるか」（ウ・エ・オ＝列A・緑）と、「登記記録・保存にどう扱われるか」（ア・イ＝列B・青）の2系統に分け、通し番号1〜5の5枚のカードで一枚に俯瞰できる構成にした。導入イラスト・導入文のブロックは置かず、タイトル・サブタイトル直後からカード群に入る。

```
Create a Japanese-language infographic, portrait layout, 1080x1920 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), rounded card sections, consistent with a
modern explainer-graphic aesthetic (icons: an isometric building being
demolished into an empty land plot, archive boxes, registry-window
counters, family/heir figures, apartment buildings — adapt icon set to
the topic of a building disappearing and being registered as such).

GLANCEABLE-POSTER REQUIREMENT (critical): This is a quick-reference poster,
NOT a text-heavy explainer document. There is NO intro illustration and NO
paragraph of prose anywhere on this poster — go straight from the header
to the cards. Every card must communicate its point almost entirely
through the illustration (icons, X marks, checkmarks, small embedded
labels) plus one short heading and one short conclusion tag. Do NOT render
any full-sentence explanation, legal citation, or paragraph of body text
anywhere on the poster. If a piece of information cannot be expressed as a
short label (a few words) or drawn as an icon, leave it out rather than
writing it as prose.

CRITICAL TEXT REQUIREMENT: All text must be rendered in standard Japanese
only — hiragana, katakana, and Jōyō (regular Japanese) kanji. Do NOT use
Simplified Chinese characters (simplified hanzi) under any circumstances,
even if a character looks similar. Every kanji must match standard Japanese
orthography exactly as written below, stroke-for-stroke. Reproduce the
exact text strings given below verbatim — do not paraphrase, translate,
summarize, or substitute any characters. Pay special attention to the
kanji 登・記・権・建・物・所・続 — these must be rendered in their standard
Japanese forms, never as Simplified Chinese variants (権 must not become
权; 記 must not become 记).

--- HEADER ---
Title (large, bold, 1行):
建物の滅失登記、5つの重要ルール

Subtitle (smaller, centered, 1行):
令和2年度 午後の部 第17問－申請人の範囲と登記記録の扱い

（タイトル・サブタイトルのすぐ下にカード群を続ける。導入イラスト・導入文の
ブロックは置かない。）

--- COLUMN A HEADER (pill-shaped badge, color: green) ---
誰が申請人になれるか

--- COLUMN A, CARD 1 ---
Badge: a filled green circle containing the number 1.
Heading (bold, ONE line, ~20 characters or fewer):
仮登記名義人は滅失登記を申請不可
Illustration: A burnt/damaged building icon; a person icon labeled
仮登記名義人 holding a 消防署の証明書 and a 承諾書 but blocked by a red
"×" barrier in front of the registry-window icon; beside it, a separate
person icon labeled 所有権の登記名義人 passes through the same window
with a green checkmark.
Conclusion tag (green banner below the illustration, 5-15 characters):
申請人になれない

--- COLUMN A, CARD 2 ---
Badge: a filled green circle containing the number 2.
Heading (bold, ONE line, ~20 characters or fewer):
相続人は相続登記なしで申請可
Illustration: A grayed-out silhouette icon (亡くなった所有権の登記名義人)
with an arrow pointing to a child/heir person icon (相続人); the heir
submits a document labeled 滅失の登記 directly to the registry-window
icon with a green checkmark, while a separate icon labeled 相続による
所有権移転の登記 is shown crossed out with a dashed arrow bypassing it.
Conclusion tag (green banner below the illustration, 5-15 characters):
相続登記は不要

--- COLUMN A, CARD 3 ---
Badge: a filled green circle containing the number 3.
Heading (bold, ONE line, ~20 characters or fewer):
一棟の滅失は区分所有者の1人から
Illustration: A large isometric apartment building (一棟の建物) with
several distinct owner icons (区分所有者) standing beside it; one single
owner icon steps forward alone and submits a document labeled 一棟の建物
の滅失の登記 with a green checkmark, while the other owner icons remain
passively in the background.
Conclusion tag (green banner below the illustration, 5-15 characters):
1人から申請可

--- COLUMN B HEADER (pill-shaped badge, color: blue) ---
登記記録・保存にどう扱われるか

--- COLUMN B, CARD 4 ---
Badge: a filled blue circle containing the number 4.
Heading (bold, ONE line, ~20 characters or fewer):
抵当権消滅の承諾で土地にも記録
Illustration: An isometric condo unit (区分建物) linked by a dotted line to
a land plot icon labeled 敷地権; the unit has a red tag icon labeled
抵当権; a document icon labeled 承諾情報 is handed from a person icon
（抵当権の登記名義人）toward the land plot; after that, the land plot's
抵当権 tag fades away with a stamp reading 消滅.
Conclusion tag (blue banner below the illustration, 5-15 characters):
消滅した旨を記録

--- COLUMN B, CARD 5 ---
Badge: a filled blue circle containing the number 5.
Heading (bold, ONE line, ~20 characters or fewer):
滅失登記の書類は30年間保存
Illustration: A filing cabinet or archive box icon at a registry office
(登記所), with a document labeled 滅失の登記の申請情報 being placed inside;
a calendar/clock icon beside it displays "30年間".
Conclusion tag (blue banner below the illustration, 5-15 characters):
30年間保存

--- FOOTER ---

Card list for self-verification (do not duplicate, omit, or reword any of
these 5 headings):
1. 仮登記名義人は滅失登記を申請不可
2. 相続人は相続登記なしで申請可
3. 一棟の滅失は区分所有者の1人から
4. 抵当権消滅の承諾で土地にも記録
5. 滅失登記の書類は30年間保存

Final check before rendering: scan every kanji glyph and confirm it is
standard Japanese (Jōyō) form, not Simplified Chinese. If any character
renders as a Simplified Chinese variant, redraw that character in the
correct Japanese form. Confirm the number of cards equals 5 exactly, with
no duplicated or missing cards, that badge numbers run 1-5 continuously
across both columns without resetting, confirm there is no intro
illustration or paragraph block between the header and the cards, and
confirm that no card contains a full sentence of explanatory prose —
every card's takeaway must read as a short heading + a short conclusion
tag, at a glance.
```
