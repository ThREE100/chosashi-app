## 【土地家屋調査士受験生向け】令和6年度 第9問〜地目が同じでも現況が違えば合筆不可〜
...
国、地方公共団体その他の公共法人が官庁又は公署として嘱託によってする登記には、当事者による申請とは異なる特則がいくつか定められています。地方公共団体が所有する土地について合筆の登記を嘱託する場合も、登記識別情報の提供を要しません。
...
- **ア（正）** 持分割合が異なる土地は合筆できない
- **イ（誤）** 登記記録の地目が同一でも現況地目が異なれば合筆できない
- **ウ（正）** 地方公共団体の嘱託登記に登記識別情報は不要
- **エ（誤）** 所有権登記がない土地とある土地は合筆できない
- **オ（正）** 承役地への合筆には地役権設定の範囲を申請情報に記載
...
- 各肢の根拠は、ア（法41条4号）・イ（法41条2号）・エ（法41条5号）・オ（令別表9項）については、いずれもローカル法令データベースの条文原文と照合済みです。ウ（地方公共団体の嘱託登記における登記識別情報の要否）については、正誤判定自体は土地家屋調査士試験対策アプリの検証済みデータベース（takuitsu.json）で確認済みですが、根拠として当初挙げられていた法22条ただし書・令7条2項・16条4項・18条4項・準則87条3項・登録免許税法4条1項を条文原文で個別に照合したところ、令7条2項は「国の機関の所管に属する権利について命令又は規則により指定された官庁又は公署の職員」による嘱託の場合に法人資格証明情報・代理権限証明情報の提供を除外するのみで地方公共団体一般の嘱託には及ばず、令16条4項・18条4項は印鑑証明書の添付を要しないとする規定であって登記識別情報とは無関係、準則87条3項は建物の表題登記における所有権証明情報の省略に関する規定で本問（土地の合筆登記における登記識別情報）とは対象が異なり、登録免許税法4条1項は登録免許税の非課税規定であって、いずれも「登記識別情報の提供を要しない」という結論を条文レベルで直接裏付けるものではないことが判明したため、本文・確認事項ともこれらの条文番号の記載を削除しました。地方公共団体が合筆登記を嘱託する場合に登記識別情報の提供が不要となる直接の根拠条文は、ローカル法令データベースおよび本環境からアクセス可能な範囲のWeb検索（e-Gov・法務局サイト等への直接アクセスがネットワーク制限によりブロックされているため確認できる範囲に限りがあります）では特定できませんでした。結論自体は公式正答（takuitsu.json）と整合していますが、条文根拠は先例・通達等に基づく可能性があり、公開前に別途一次資料での確認を推奨します。オについては、条文が定めているのは「地役権設定の範囲」を申請情報の内容とすべきことであり（不動産登記令別表9の項申請情報欄ロ）、データベース・旧本文にあった「添付情報」という引用先の欄名が実際とは異なっていたため、本文を「申請情報」に訂正しました。
...
- **最新法令チェック（2026-08-16実施）**：本問の論点（合筆の制限・嘱託登記の特則）について、相続登記義務化等の直近の主要改正による影響を確認しましたが、不動産登記法41条の合筆の制限そのものへの影響は見つかりませんでした。
...
合筆の登記に関する5肢を、「合筆を妨げる要件（持分・地目・権利関係・地役権の範囲）」と「嘱託登記の特則」という2つの軸に沿って、誤りの肢（イ・エ）は正しいルールに直したうえで、通し番号バッジ・短い見出し・図解・短い結論タグだけで構成する2列・5枚のポスター型カードで俯瞰する構成。導入イラストや説明文のブロックは置かない。
...
令和6年度 午後の部 第9問－合筆を妨げる要件と嘱託登記の特則
...
Subtitle (smaller, centered, 1行):
令和6年度 午後の部 第9問 作図ガイド（合筆の登記の制限と記載事項）
