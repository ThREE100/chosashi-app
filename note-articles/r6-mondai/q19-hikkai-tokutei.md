...
- 各肢の条文根拠のうち、ア（不動産登記法123条1号・132条1項5号）、イ（不動産登記規則209条1項4号）、オ（不動産登記規則207条3項7号・不動産登記法132条1項6号）は、データベースのexplanationフィールドで条文番号まで明記されています。ウ（時効取得した部分が対象筆界に接していなくても申請できる点）についても、不動産登記規則209条1項5号が添付情報として求めるのは「一筆の土地の一部について所有権を取得したことを証する情報」のみであり、この号が当該部分が対象筆界に接することを要件としていないことから、条文上も裏付けが取れることを確認しました。エ（双方表題登記のない土地どうしは対象にならない点）は、explanationフィールドでは「筆界」の定義（アで示した条文）から導かれる一般原則としての説明にとどまり、個別の条文番号までは明記されていないため、断定的な条文引用は避けています。
