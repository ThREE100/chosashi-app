## 【土地家屋調査士受験生向け】平成17年度 第11問〜換地で分かれても、所有者に分筆義務はないんです〜

**出題年度：平成17年度　午後の部　第11問**

> 土地区画整理事業施行区域内の土地の登記手続に関する次のアからオまでの記述のうち、正しいものは何個あるか。
>
> ア　仮換地上の建物の表題登記を申請する場合、建物の所在には現在の地番を記載し、予定地番がある場合には、これを括弧書きで併記することができる。
>
> イ　換地処分の公告があった後、換地計画において換地を定めなかった従前の宅地についての権利は消滅し、当該従前の宅地についての登記記録は閉鎖される。
>
> ウ　施行者は、表示に関する登記がない従前の土地について、所有者に代位して表題登記の申請をすることができる。
>
> エ　従前の1筆の土地に対して数個の換地が定められた場合、当該土地の所有者は、分筆の登記を申請する義務を負う。
>
> オ　施行者は、換地処分前であっても、保留地について表題登記の申請をすることができる。
>
> 1　1個　　2　2個　　3　3個　　4　4個　　5　5個

土地区画整理事業に関する登記は、土地家屋調査士試験のなかでも実務色が強く、「誰が」「いつ」「何を」申請できるのかが細かく問われる分野です。仮換地・換地処分・保留地という3つの場面ごとに登記のルールが異なるため、混同しやすいのが特徴です。この問題は「正しいものが何個あるか」を問う個数問題なので、ア〜オを一つずつ丁寧に確認していきましょう。

### ア：仮換地上の建物の表題登記は、現地番に予定地番を括弧書きで併記できる

仮換地の指定を受けた土地の上に建物が新築された場合でも、建物の表題登記における「所在」は、原則として従前の土地（底地）の現在の地番を用いて記載します。もっとも、登記実務上は、この現地番に続けて、将来その土地が換地されたときの予定地番を括弧書きで併記することが認められています。これは、区画整理事業が完了して換地処分が行われるまでの間、建物の所在を分かりやすく特定するための実務上の取扱いです。したがって、肢アは正しい記述です。

**たとえば**、仮換地の指定を受けた土地にＢさんが自宅を新築し、建物の表題登記を申請するとします。このとき、所在欄には現在の地番（底地の地番）をそのまま書き、その後ろに「（換地予定地番　○番）」のようにカッコ書きで将来の地番を書き添えることができます。こうしておけば、後日その土地が正式に換地されたときも、どの建物についての登記だったかが一目で分かります。

### イ：換地を定めなかった従前の宅地の権利は、公告日の終了時に消滅し登記記録も閉鎖される

換地処分の公告があった場合、換地計画において換地が定められた宅地は、公告があった日の翌日から従前の宅地とみなされて権利が引き継がれます。これに対し、換地計画において対応する換地の部分を定められなかった従前の宅地についての権利は、公告があった日が終了した時に消滅します（土地区画整理法104条）。権利が消滅した以上、その従前の宅地についての登記記録も、換地処分に伴う登記の中で閉鎖されることになります。したがって、肢イは正しい記述です。

**たとえば**、Ａさんが所有していた従前の宅地が、区画整理の結果、道路や公園などの公共用地として使われることになり、対応する換地が定められなかったとします。この場合、換地処分の公告があった日が終わった時点で、Ａさんがその従前の宅地について持っていた所有権は消滅し、その土地の登記記録も閉鎖されます。

### ウ：施行者は、表示登記のない従前地について所有者に代位して表題登記を申請できる

区画整理事業の施行地区内には、もともと表示に関する登記がされていない土地（未登記地）が含まれていることがあります。区画整理法は、こうした土地についても円滑に換地処分に伴う登記を進められるよう、施行者が本来の申請義務者である所有者に代わって（代位して）、表題登記を申請することを認めています（土地区画整理法107条4項の委任に基づく土地区画整理登記令2条1項1号）。大量の土地・建物について一斉に登記処理をする必要がある区画整理事業の特性上、施行者にこの代位申請権を与えることで手続の停滞を防いでいます。したがって、肢ウは正しい記述です。

**たとえば**、区画整理事業の対象地内に、表題登記がされていない古い倉庫が建っていたとします。所有者が自分で法務局に出向いて表題登記の手続をしなくても、施行者がその所有者に代わって倉庫の表題登記を申請することができる、というのがこの規定の趣旨です。

### エ：数個の換地が定められても、所有者に分筆登記の申請義務はない

従前の1筆の土地に対して数個の換地が定められた場合、その換地に対応させる分筆の手続は、施行者が所有者に代わって（代位して）行うことができます（土地区画整理法82条1項）。区画整理事業の施行のために必要な範囲で施行者にこの代位の権限が認められているため、所有者自身が改めて分筆登記を申請する義務を負うことはありません。したがって、「所有者は分筆の登記を申請する義務を負う」とする肢エは誤りです。

**たとえば**、Ａさんが持っていた従前の1筆の土地が、区画整理の結果、2つの換地に分かれて割り当てられたとします。この場合でも、Ａさんが自分で法務局に出向いて分筆登記を申請する必要はなく、施行者がＡさんに代わって分筆の手続を行います。

### オ：保留地の表題登記は、換地処分後（公告日の翌日以後）でなければ申請できない

保留地は、仮換地とは異なり、対応する従前の宅地が存在しない土地です。そのため、換地処分によって初めて土地として確定するのであって、換地処分前の段階では登記の対象として観念することができません。施行者が保留地を取得するのは換地処分の公告があった日の翌日であり（土地区画整理法104条11項）、保留地についての表題登記も、この換地処分の公告があった後に初めて申請することができます。したがって、「換地処分前であっても表題登記の申請をすることができる」とする肢オは誤りです。

**たとえば**、区画整理事業の費用に充てるために留保されている保留地があるとします。この土地は、換地処分の公告があるまでは正式な地番も定まっていない状態なので、施行者はそれより前の段階で保留地の表題登記を申請することはできません。

### まとめ

- **ア（正）** 仮換地上の建物の表題登記は、現地番に予定地番を括弧書きで併記できる（実務上の取扱い）
- **イ（正）** 換地を定めなかった従前の宅地の権利は公告日終了時に消滅し、登記記録も閉鎖される
- **ウ（正）** 施行者は表示登記のない従前地について、所有者に代位して表題登記を申請できる
- **エ（誤）** 分筆の手続は施行者が所有者に代わって（代位して）行うことができ、所有者に申請義務はない
- **オ（誤）** 保留地の表題登記は換地処分後（公告日の翌日以後）でなければ申請できない

正しいのはア・イ・ウの3つで、エ・オは誤りです。区画整理の登記は、仮換地の段階・換地処分の段階・保留地の取扱いという3つの局面ごとに、「誰が」「いつ」登記できるのかを正確に押さえておくことが得点の分かれ目になります。

**正解：3個（選択肢3番）**

---

**このまま使える点／使う前に確認したい点**

- 文章としてはこのままnoteに貼り付けて投稿できる内容です。
- 出題年度（平成17年度午後の部第11問）・正解（正しいものは3個＝選択肢3番）は、土地家屋調査士試験対策アプリの検証済みデータベース（この年度はtakuitsu.jsonのexplanationが空欄のため、主にkaisetsu_plus.jsonのapproach／pitfalls／keyPoints）で確認済みです。
- 条文レベルで確認できたもの：肢イ（土地区画整理法104条・換地を定めなかった従前の宅地の権利消滅）はWikibooksで条文原文の内容と一致することを確認しました。肢オの「保留地は施行者が換地処分の公告があった日の翌日に取得する」という時期（104条11項）についても、複数の解説サイトで一致して確認できました。
- データベースのkeyPointsでは、肢ウの根拠を「107条の2」、保留地関連を「108条（換地処分後の登記の嘱託）」としていましたが、WebSearchで確認したところ、「107条の2」という枝番の条文の存在は確認できず、施行者による代位申請の内容は複数の解説サイトで「107条3項」相当の規定として説明されているものが見つかりました。また「108条」は「保留地等の処分」を定める条文であり、換地処分に伴う登記の申請・嘱託義務を定めているのは107条（1項・2項）であることが確認できました。この食い違いを踏まえ、本文では肢ウの根拠を「土地区画整理法107条」とだけ記載し、DBが挙げていた「107条の2」「108条」という具体的な項番号はそのままでは採用していません。
- **検証で判明した肢ウの正確な根拠（2026-09-16実施、`laws/tochi-kukaku-seiri-hou.md`により再確認）**：条文原文にあたって再検証した結果、施行者が未登記の従前地について所有者に代位して表題登記を申請できる根拠は、**土地区画整理法107条4項の委任に基づく土地区画整理登記令2条1項1号**（「施行者は…不動産の表題登記を（所有者に代わって）申請することができる」）であることを確認しました。一方、107条3項は「2項の登記がされるまで他の登記はできない」という別内容の規定であり、代位申請の根拠ではないことも判明しました。データベースの「107条の2」という表記は誤りです。
- **検証で判明した肢ア（2026-09-16実施）**：仮換地上の建物の表題登記における現地番・予定地番の括弧書き併記は、不動産登記法・不動産登記規則・不動産登記事務取扱手続準則のいずれにも明文規定がなく、次の2つの先例・通達に基づく実務であることが、複数の先例集サイトの引用要旨により確認できました。昭和34年7月10日建設計発374号建設省計画局長通達（法務省と協議済み。所在地番は底地の地番を記載し、換地の予定地番を括弧書きで併記するという枠組みの出発点）、および昭和43年2月14日民事甲第170号民事局長回答（底地の現在の地番を記載し、予定地番があれば括弧書きにより併記することができる、という内容）。この論点を深掘りした個別記事`note-articles/column/karikanchi-tatemono-shozai-nejire.md`もご参照ください。
- **検証で判明した肢エ（2026-09-16実施、`laws/tochi-kukaku-seiri-hou.md`により再確認）**：本文で述べた「分筆の手続を施行者が所有者に代わって（代位して）行うことができる」という根拠は、**土地区画整理法82条1項**（「施行者は、土地区画整理事業の施行のために必要がある場合においては、所有者に代わつて土地の分割又は合併の手続をすることができる。」）であることを条文原文で確認しました。以前の版で「施行者からの嘱託または登記所の職権」としていた説明は不正確だったため、上記のとおり修正しています（嘱託・職権はこれとは別の場面の話です）。なお、土地区画整理登記令の全26条を通読した結果、「分筆登記」「合筆登記」という名称の独立した条文はこの政令に存在せず、換地処分に伴う土地の権利関係の変動そのものは、107条2項を根拠に施行者が行う包括的な「換地処分による土地の登記」（登記令4条・10条により、施行地区内の土地全部について一の申請情報で一括申請する）の中で処理される仕組みです。従前の数個の土地が1個の換地にまとまる場合の所有権の登記に限り、登記令11条により登記官が職権で行います。これは82条1項の分筆・合筆の手続とは別の、換地処分後の最終登記段階の話であり、「所有者に分筆の登記を申請する義務はない」という肢エの結論自体は変わりません。
- アガルート等のローカル教材PDFは本実行環境に存在せず、参照していません。本記事は上記アプリの検証済みデータベースと、WebSearchで確認できた範囲の条文・実務解説のみに基づいて作成しています。
- **QAチェックリスト再検証（2026-09-19実施）**：`note-articles/qa-checklist.md`の全19項目（A〜G）に基づき再検証しました。A（`laws/tochi-kukaku-seiri-hou.md`の82条1項（肢エ）・104条1項（肢イ）・104条11項（肢オ）・107条（肢ウの前提）の条文原文を改めて突合し、いずれも本文・既存の確認事項ブロックの記述と一致することを確認。判例・先例・専門誌番号は本文に記載していないことを確認。試験範囲外の一般法は使用していない）、B（正解＝3個・選択肢3番を再確認し、まとめの正誤表記と整合していることを確認）、C（見出しが正しい結論を表していること、正解の先出しがないこと、敬体で統一されていること、条文解釈プロセスの説明がないこと、全角括弧で統一されていることを確認）、D（Markdown表の不使用を確認）、E（インフォグラフィックの5枚のカード見出し・結論タグが本文の各肢の判定・内容と一致していることを確認）、F（テンプレート構造・タイトルのキャッチフレーズが25字以内・確認事項ブロックの記載を確認）、G（`src/data/takuitsu.json`で「区画整理」を検索したところ、H18問8・H20問7・H23問11・R1問14・R5問5・R5問7等、区画整理事業に関する登記を主題とする問題は他年度にも複数ありますが、いずれも肢構成・具体的な論点（換地処分の効果、代位申請、保留地の取扱い等の組み合わせ）が本問とは異なり、同一肢構成の出題ではないため、独立記事として問題ないと判断しました。最新法令チェックとして、本文が根拠とする土地区画整理法82条・104条・107条に、正誤判定へ影響する近年の改正は確認していません）を行い、修正すべき誤りは見つかりませんでした。

---

## 見出し画像用フレーズ

- 仮換地の建物、地番はカッコ書きでOKなんです
- 換地がない従前地、権利は消えて登記も閉じるんです
- 表題登記、施行者が代わりに申請できるって知ってた？
- 分筆登記、所有者が申請しなくていいんです
- 保留地の登記、換地処分のあとからなんです

---

## インフォグラフィック プロンプト（問題全体）

土地区画整理事業の登記について「誰が登記するか」と「いつ登記できるか」という2つの軸に沿って、5肢すべての正しいルールを、通し番号バッジ・短い見出し・図解・短い結論タグだけで構成する2列・5枚のポスター型カードで俯瞰する構成。

```
Create a Japanese-language infographic, portrait layout, 1080x1920 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), rounded card sections, consistent with a
modern explainer-graphic aesthetic (icons: isometric buildings, land
plots grouped in grid blocks like a land readjustment zone, official
stamps, document/paper icons, torn paper labels, etc. — adapt icon set
to the topic).

GLANCEABLE-POSTER REQUIREMENT (critical): This is a quick-reference poster,
NOT a text-heavy explainer document. There is NO intro illustration and NO
paragraph of prose anywhere on this poster — go straight from the header
to the cards. Every card must communicate its point almost entirely
through the illustration (icons, X marks, checkmarks, small embedded
labels) plus one short heading and one short conclusion tag. Do NOT render
any full-sentence explanation, legal citation, or paragraph of body text
anywhere on the poster. If a piece of information cannot be expressed as a
short label (a few words) or drawn as an icon, leave it out rather than
writing it as prose.

CRITICAL TEXT REQUIREMENT: All text must be rendered in standard Japanese
only — hiragana, katakana, and Jōyō (regular Japanese) kanji. Do NOT use
Simplified Chinese characters (simplified hanzi) under any circumstances,
even if a character looks similar. Every kanji must match standard Japanese
orthography exactly as written below, stroke-for-stroke. Reproduce the
exact text strings given below verbatim — do not paraphrase, translate,
summarize, or substitute any characters. Pay special attention to the
kanji 権・地・番・建・物・登・記・所, which appear repeatedly below (地番,
予定地番, 登記, 表題登記, 登記記録, 登記所, 権利, 建物, 施行者 など) —
render each in its standard Japanese (Jōyō) form, never the Simplified
Chinese variant.

--- HEADER ---
Title (large, bold, 1行):
その分筆、所有者が申請しなくていいんです

Subtitle (smaller, centered, 1行):
平成17年度 午後の部 第11問－土地区画整理事業と表示登記の急所

（タイトル・サブタイトルのすぐ下にカード群を続ける。導入イラスト・導入文の
ブロックは置かない。）

--- COLUMN A HEADER (pill-shaped badge, color: green) ---
誰が登記するか

--- COLUMN A, CARD 1 ---
Badge: a filled green circle containing the number 1.
Heading (bold, ONE line, ~20 characters or fewer):
未登記地は施行者が代位申請
Illustration: An isometric character wearing a construction helmet
(labeled 施行者) holding a document labeled 代位申請 and submitting it
at a government building icon (法務局). A second character labeled
所有者 stands behind, simply watching, not holding any document.
Conclusion tag (green banner below the illustration, 5-15 characters):
代位申請OK

--- COLUMN A, CARD 2 ---
Badge: a filled green circle containing the number 2.
Heading (bold, ONE line, ~20 characters or fewer):
分筆は所有者の義務ではない
Illustration: A single land-plot icon split by a dashed line into two
plots. Next to it, an isometric character wearing a construction helmet
(labeled 施行者) holds a document labeled 分筆の手続. The 所有者
character stands apart with a red prohibition (no-entry) mark over a
label reading 申請不要.
Conclusion tag (green banner below the illustration, 5-15 characters):
申請義務なし

--- COLUMN B HEADER (pill-shaped badge, color: blue) ---
いつ登記できるか

--- COLUMN B, CARD 3 ---
Badge: a filled blue circle containing the number 3.
Heading (bold, ONE line, ~20 characters or fewer):
仮換地建物は予定地番も併記可
Illustration: An isometric house standing on a plot labeled 仮換地.
Next to the house, a document/label icon shows the text
「所在：現在の地番（予定地番　○番）」, with the bracketed part
(予定地番　○番) enclosed in a dashed outline to emphasize that it is
written in parentheses alongside the current lot number.
Conclusion tag (blue banner below the illustration, 5-15 characters):
括弧書きでOK

--- COLUMN B, CARD 4 ---
Badge: a filled blue circle containing the number 4.
Heading (bold, ONE line, ~20 characters or fewer):
換地なし従前地は権利消滅
Illustration: A two-panel timeline. Left panel: a land plot icon
labeled 従前の宅地 with an official stamp reading 公告 being pressed
onto it. Right panel: the same plot icon now grayed out/faded, next
to a registry-book icon stamped in red with 閉鎖.
Conclusion tag (blue banner below the illustration, 5-15 characters):
登記記録も閉鎖

--- COLUMN B, CARD 5 ---
Badge: a filled blue circle containing the number 5.
Heading (bold, ONE line, ~20 characters or fewer):
保留地は換地処分後に登記可
Illustration: A land-plot icon labeled 保留地 with a padlock icon
overlaid on it. A horizontal timeline arrow beneath shows three
stages in order: 換地処分の公告 → 翌日 → 表題登記が可能。 Before the
first stage, the plot icon carries a red prohibition mark labeled
表題登記不可.
Conclusion tag (blue banner below the illustration, 5-15 characters):
公告後に申請可

--- FOOTER ---

Final check before rendering: scan every kanji glyph and confirm it is
standard Japanese (Jōyō) form, not Simplified Chinese. Pay special
attention to 権・地・番・建・物・登・記・所 in words such as 地番,
予定地番, 登記, 表題登記, 登記記録, 登記所, 権利, 建物, 施行者 — if any
character renders as a Simplified Chinese variant, redraw that character
in the correct Japanese form. Confirm the number of cards equals 5
exactly, with no duplicated or missing cards, that badge numbers run 1-5
continuously across both columns without resetting, confirm there is no
intro illustration or paragraph block between the header and the cards,
and confirm that no card contains a full sentence of explanatory prose.
```

## インフォグラフィック プロンプト（ア〜オ 作図ガイド）

```
Create a Japanese-language infographic, portrait layout, 1080x2600 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), rounded panel sections, consistent with the
same visual language as the whole-problem poster for this article, but
built as a set of 5 diagram-drawing panels (a "how to sketch this fact
pattern, in the right order" study reference) rather than a
quick-reference conclusion poster.

DIAGRAM-GUIDE REQUIREMENT (critical): Each panel's purpose is to show the
reader exactly what diagram they should draw on scratch paper while
reading this type of problem, AND the order in which they should check
conditions to get there — an isometric 仮換地 land-plot icon, a 従前の
宅地 land-plot icon, a 保留地 land-plot icon, an isometric 施行者 figure
wearing a construction helmet, an isometric 所有者 figure, a registry-
office building icon (法務局), a calendar/stamp icon for 公告, a closed
registry-book icon, a land plot split by a dashed line for 分筆, and a
document icon with bracketed text for 予定地番. Where a 肢 requires
checking multiple conditions in sequence before reaching a conclusion,
draw the panel's diagram as an actual decision flowchart: diamond-shaped
branch nodes with the condition written on them, Yes/No (or ○/✕) branch
arrows, and a final conclusion node. Where a 肢 is resolved by a single
check, a labeled illustrative diagram is sufficient — do not force a
flowchart. Panel 2（肢イ）is a decision tree where BOTH outcomes carry
real, distinct legal conclusions: draw a diamond node 換地計画で対応する
換地が定められたか？with a「はい」branch leading to its own conclusion
node (公告翌日から従前の宅地とみなされ権利が引き継がれる、通常の重みで
描く) and a「いいえ」branch (このパネルの主眼のため太い縁取り・フルカラー
で強調) leading to a separate, explicit conclusion node (公告があった日
が終了した時に権利消滅、登記記録も閉鎖される); do not draw any arrow
that loops back from either leaf to an earlier node. Unlike a glanceable
summary poster, each panel MAY include a short「着眼点」callout box with
1-2 sentences that state the checking ORDER in words, not just the
conclusion. Do not include case or precedent numbers (article/regulation
numbers are fine); keep the callout text as written below verbatim, and
keep every condition each callout describes faithful to the article's
own body text.

CRITICAL TEXT REQUIREMENT: All text must be rendered in standard Japanese
only — hiragana, katakana, and Jōyō (regular Japanese) kanji. Do NOT use
Simplified Chinese characters (simplified hanzi) under any circumstances,
even if a character looks similar. Do NOT use Traditional Chinese
characters (traditional hanzi) either, even where a traditional-hanzi
glyph looks close to the correct Japanese kanji form — every glyph must
match the standard Japanese Jōyō form exactly, not the Chinese
traditional variant. Do NOT render any character that is not standard
Japanese hiragana, katakana, or Jōyō kanji anywhere in the image —
no Chinese-only characters, no Korean Hangul, no other non-Japanese
script, and no stray or decorative glyphs of any kind, even as small
background or texture elements. Reproduce the exact text strings given
below verbatim — do not paraphrase, translate, summarize, or substitute
any characters. Within this English prompt text, use half-width
parentheses ( ) consistently — never open a parenthetical with a
full-width （ and close it with a half-width ), or vice versa.

BACKGROUND REQUIREMENT (critical): The entire canvas must be fully opaque
from edge to edge. Do NOT generate a transparent or alpha-channel
background under any circumstances, even if the output file format
supports transparency. Fill the full canvas — including every corner and
margin outside the panels — with a solid or illustrated opaque background
(the pale beige/gray tone used elsewhere in this style is a good
default). There must be no checkerboard pattern, no partially transparent
area, and no unpainted canvas edge anywhere in the final image.

--- HEADER ---
Title (large, bold, 2行):
問題文を読んだら
どんな図を描けばいいか

Subtitle (smaller, centered, 1行):
平成17年度午後第11問 作図ガイド（土地区画整理事業の登記）

（タイトル・サブタイトルのすぐ下にパネル群を続ける。導入イラスト・導入文の
ブロックは置かない。）

--- PANEL 1（肢ア） ---
Badge: a filled circle in green containing the number 1.
Heading (bold, ONE line):
仮換地上の建物は現地番と予定地番を併記
Diagram: A single labeled illustrative diagram (this 肢 is resolved by
one check, so no flowchart is needed). An isometric house standing on a
land plot labeled 仮換地. Beside the house, a document icon shows two
stacked step callouts numbered ①② read top to bottom: ①「まず現在の地番
（底地の地番）を記載」next to the text「所在：現在の地番」, ②「次に予定
地番があれば括弧書きで併記」next to the text「（予定地番　○番）」, with
the bracketed part enclosed in a dashed outline to show it is added
afterward, not instead of the current lot number.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、建物の所在には現在の地番（底地の地番）を記載することを確認しま
す。次に、将来の換地の予定地番があれば、それを括弧書きで現地番の後ろ
に併記できることを確認します。
Conclusion tag (a short colored banner/pill, green, 5-15 Japanese
characters):
現地番＋括弧書きでOK

--- PANEL 2（肢イ） ---
Badge: a filled circle in blue containing the number 2.
Heading (bold, ONE line):
換地なしの従前地は公告終了時に消滅
Diagram: A decision-tree flowchart. Diamond node: 換地計画で、この従前
の宅地に対応する換地が定められたか？The「はい」branch leads to a
conclusion node (drawn with normal weight, not this panel's focus):
公告の翌日から換地が従前の宅地とみなされ、権利が引き継がれる。The
「いいえ」branch (highlighted with a thick colored border and full
color, since this is the panel's focus) leads to a separate, clearly
distinct conclusion node: 公告があった日が終了した時に権利消滅、登記
記録も閉鎖される, illustrated with a land-plot icon labeled 従前の宅
地 turning grayed-out beside a registry-book icon stamped in red with
閉鎖. No arrow loops back from either conclusion node to the diamond.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、この従前の宅地に対応する換地が定められたかどうかを確認します。
定められなかった宅地は、公告があった日が終了した時に権利が消滅し、
登記記録も閉鎖されることを確認します。
Conclusion tag (a short colored banner/pill, blue, 5-15 Japanese
characters):
権利消滅・登記記録閉鎖

--- PANEL 3（肢ウ） ---
Badge: a filled circle in green containing the number 3.
Heading (bold, ONE line):
未登記の従前地は施行者が代位して申請
Diagram: A 系統図・関係図型 diagram showing the relationship between
three parties, drawn top to bottom as two numbered steps. ①最初の確認
点として、従前の土地のアイコンに「表示登記なし」というラベルを添え
る。②次に、ヘルメット姿の施行者のアイコンから、法務局（登記所）の建
物アイコンへ向かって「代位して表題登記を申請」というラベル付きの矢
印を伸ばす。所有者のアイコンは脇に立ち、書類を何も持たずにただ見て
いる姿で描く。
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、この従前の土地に表示に関する登記があるかどうかを確認します。
登記がない場合は、施行者が所有者に代わって（代位して）表題登記を申
請できることを確認します。
Conclusion tag (a short colored banner/pill, green, 5-15 Japanese
characters):
施行者が代位申請可

--- PANEL 4（肢エ） ---
Badge: a filled circle in blue containing the number 4.
Heading (bold, ONE line):
分筆の申請義務は所有者でなく施行者が代位
Diagram: A 正誤対比型 split panel. LEFT side (誤りやすい思い込み):
所有者のアイコンが分筆登記の申請書を法務局へ提出しようとしているイラ
ストに「所有者に申請義務がある」というラベルを重ね、大きな赤い×印と
取り消し線を描く。RIGHT side (正しいルール、太い縁取りで強調): ヘル
メット姿の施行者のアイコンが代わりに分筆の手続書類を提出しているイラ
ストに「施行者が代位して手続」というラベルと緑のチェックマークを付け
る。所有者のアイコンは脇に立ち、何も持たずに見ているだけの姿で描く。
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、従前の1筆の土地に数個の換地が定められたことを確認します。次
に、その分筆の手続きは施行者が所有者に代わって行うことができ、所有
者自身には申請義務が課されていないことを確認します。
Conclusion tag (a short colored banner/pill, blue, 5-15 Japanese
characters):
申請義務は所有者になし

--- PANEL 5（肢オ） ---
Badge: a filled circle in green containing the number 5.
Heading (bold, ONE line):
保留地の登記は公告翌日以降にのみ可能
Diagram: A タイムライン型 diagram with three points arranged left to
right, connected by arrows. ①「換地処分前」の時点：保留地のアイコン
に錠前のアイコンと「表題登記不可」という赤い禁止マーク付きラベル。②
「換地処分の公告」の時点：カレンダーにスタンプが押されるイラスト。③
「公告の翌日」の時点：施行者のアイコンが保留地の鍵アイコンを受け取
り、「表題登記の申請が可能」という緑のチェックマーク付きラベルを添え
る。
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、この保留地について換地処分の公告があったかどうかを確認しま
す。公告があった日の翌日に施行者がその保留地を取得するため、その翌
日以降でなければ表題登記の申請はできないことを確認します。
Conclusion tag (a short colored banner/pill, green, 5-15 Japanese
characters):
公告翌日から申請可

--- FOOTER ---
Small footnote text (bottom of panel, small font, verbatim):
土地区画整理法104条1項・11項（肢イ・オ）、107条4項の委任に基づく土地
区画整理登記令2条1項1号（肢ウ）、82条1項（肢エ）に基づく整理です。肢
アの予定地番の括弧書き併記は、実務上の取扱いによるものです。

Final check before rendering: scan every kanji glyph and confirm it is
standard Japanese (Jōyō) form, not Simplified Chinese and not Traditional
Chinese, paying special attention to 権・地・番・登・記・所・施・換・筆・請・
認・従. If any character renders as a Simplified or Traditional Chinese
variant, redraw that character in the correct Japanese form. Also scan
the entire canvas for any character that is not standard Japanese
hiragana, katakana, or Jōyō kanji — including any Chinese-only character,
Korean Hangul, other non-Japanese script, or stray decorative glyph — and
remove or redraw it so that only standard Japanese text appears anywhere
in the image. Confirm the panel count equals 5 exactly, badge numbers run
1-5 continuously, there is no intro illustration or paragraph block
between the header and the panels, that Panel 2's decision tree shows
BOTH the「はい」and「いいえ」conclusion nodes explicitly with no arrow
looping back to an earlier node, that every multi-condition 肢 is drawn
as an actual flowchart with branch nodes (not a bare illustration with no
visible decision structure), that each 着眼点 callout states a checking
order rather than only a conclusion, confirm nothing is rendered below
the last panel's footnote text (no summary recap panel, no trophy or
medal icon, no re-listed ○/✕ grid of all 肢, and no additional text block
of any kind), and confirm the entire canvas, edge to edge, is filled with
a fully opaque background with no transparency or alpha channel anywhere.
```
