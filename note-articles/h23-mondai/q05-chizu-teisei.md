...
条文が「併せてしなければならない」と定めているのは逆向きの関係です。すなわち、地図訂正の申出をする場合に地積に錯誤があるときは、その申出を地積更正の登記の申請と併せてしなければならない、というものです（不動産登記規則16条2項）。地積変更の登記を申請する側に地図訂正の申出を義務づける規定はないので、向きが逆で誤りです。
...
- **イ（誤）**　義務の向きが逆。地図訂正申出の側に地積更正を併せる（規則16条2項）
- **ウ（正）**　当事者の合意（調停）だけで地番を付け替える地図訂正はできない
- **エ（誤）**　登記官は職権で地図等を訂正できる（規則16条15項）
...
- 肢イ（不動産登記規則16条2項）と肢エ（同条15項）は、ローカル法令データベース（`note-articles/laws/fudousan-touki-kisoku-1.md`）で条文原文を直接確認済みです。肢ア（共有者の一人からの申出）は、同条1項が申出権者を「表題部所有者若しくは所有権の登記名義人又はこれらの相続人その他の一般承継人」と定めていることを確認したうえで、共有者の一人から申出できる点は地図訂正の保存行為的性格からの整理にとどまります。肢ウ（地番の付替え）・肢オ（添付情報の省略）は実務上の取り扱いからの整理です。
...
- なお、肢オ（登記所備付けの地積測量図で位置の誤りを確認できる場合に、別途「誤りがあることを証する情報」の提供を要しないこと）については、規則16条5項1号が文言上は当該情報の提供を求めており、条文の字面だけでは省略の根拠を確認できませんでした。本記事は公式正答（肢オは誤り＝提供を要しない場合がある）に従っていますが、その根拠は登記実務上の取扱いによるものと理解してください。
