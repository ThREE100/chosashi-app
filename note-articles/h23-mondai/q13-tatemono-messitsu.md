## 【土地家屋調査士受験生向け】平成23年度 第13問〜材料を使い回しても元の建物は消える〜
...
建物の滅失の登記に関する5つの独立した個別ルール（材料流用時の扱い・承諾書の要否・区分建物での申請人・錯誤処理・滅失の判断基準）であり2つの軸には分かれないため、通し番号バッジ・短い見出し・図解・短い結論タグだけで構成する単一列・5枚のポスター型カードで俯瞰する構成。

```
Create a Japanese-language infographic, portrait layout, 1080x1920 pixels,
clean flat-design isometric illustration style with soft pastel colors
(blue, green, beige, gray), rounded card sections, consistent with a
modern explainer-graphic aesthetic (icons: isometric buildings, rubble,
fire and smoke, stamps, torn paper labels, etc. — adapt icon set to the
topic).
...
kanji 滅・証・権・構 — these must be rendered in their standard Japanese
...
Heading (bold, ONE line, ~20 characters or fewer):
実体のない区分建物は錯誤で処理
Illustration: A building registered as two separate units but shown
with no dividing wall inside, with a 錯誤 stamp applied to erase the
...
attention to 滅・証・権・構. If any character renders as a Simplified
