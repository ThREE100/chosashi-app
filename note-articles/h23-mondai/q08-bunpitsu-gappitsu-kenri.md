## 【土地家屋調査士受験生向け】平成23年度 第8問〜抵当権を消す分筆、錯誤では戻せない〜
...
- **ア（正）**　抵当権を消すには、その抵当権を目的とする第三者の承諾も要る
- **イ（正）**　範囲が一部となる地役権は、地役権図面・地役権証明書で範囲を示す
- **ウ（正）**　永小作権・採石権があると条件が同一でも合筆できない
...
mortgage/easement ribbons, stamps, documents, torn paper labels, etc. —
...
平成23年度 午後の部 第8問－所有権以外の権利のある土地の分筆及び合筆
...
piece already removed via a consent stamp. An attempt to undo
everything with a 分筆錯誤 stamp is blocked by a red X.
Conclusion tag (green banner below the illustration, 5-15 characters):
錯誤では戻せない
...
final conclusion node. Where a肢 is resolved by a single check, a labeled
...
抵当権を消すには第三者の承諾も必要
Diagram: A decision-tree flowchart. Diamond node: 消滅させる抵当権を目
的とする第三者の権利(転抵当など)に関する登記があるか？with はい branch
(drawn with a thick highlighted border, since this is本肢の場面) leading
through a linked 転抵当権 ribbon icon to a conclusion node reading 抵当
権者の承諾に加え第三者の承諾も必要, and いいえ branch leading to a
separate conclusion node reading 抵当権者の承諾だけで足りる.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、消滅させようとする抵当権について、その抵当権を目的とする第三者の
権利の登記があるかどうかを確認します。ある場合は、次に、抵当権者の承諾
を証する情報に加えて、その第三者の承諾を証する情報も必要になると判断し
ます。
...
Heading (bold, ONE line):
範囲が一部なら図面と証明書が必要
Diagram: A decision-tree flowchart. Diamond node: 承役地についてする地
役権の設定の範囲が、分筆後又は合筆後の土地の一部となるか？with はい
branch (drawn with a thick highlighted border, since this is本肢の場面)
leading to a conclusion node reading 地役権の設定の範囲を記載し地役権図
面・地役権証明書の添付が必要, and いいえ branch leading to a separate
conclusion node reading 図面・証明書の添付は不要.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、承役地についてする地役権の設定の範囲が、分筆後又は合筆後の土地の
一部となるかどうかを確認します。一部となる場合は、次に、申請情報にその
範囲を記載し、地役権図面及び地役権証明書を添付する必要があると判断しま
す。
Conclusion tag (a short colored banner/pill, green, 5-15 Japanese
characters):
図面と証明書が必要
...
Heading (bold, ONE line):
永小作権・採石権は列挙にない権利
Diagram: A decision-tree flowchart shared with Panel 5's合筆制限の例外
を判定する木. Diamond node: 土地に付いている所有権以外の権利は、不動産
登記規則が合筆の例外として限定列挙する権利(承役地の地役権・条件が同一
の担保権など)に含まれるか？The いいえ branch is drawn with a thick
highlighted border pointing to a labeled box 永小作権・採石権(本肢),
leading to a conclusion node reading 登記の目的・受付年月日・受付番号・
登記原因及びその日付が同一でも合筆できない. A separate いいえ側の box
for 所有権の移転の仮登記 is drawn faded and greyed-out, since it belongs
to another肢. The はい branch is also drawn faded and greyed-out, leading
to a small conclusion node reading 条件が同一なら合筆できる場合がある,
since this branch is not本肢の対象.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、永小作権・採石権が、不動産登記規則が合筆の例外として限定的に認め
る権利の一覧に含まれているかどうかを確認します。含まれていない場合は、
次に、登記の目的・受付年月日・受付番号・登記原因及びその日付が同一であ
っても、その一致だけでは合筆できないと判断します。
...
けを理由にその登記を抹消できるかを確認し、抹消は申請できないと判断しま
す。
Conclusion tag (a short colored banner/pill, green, 5-15 Japanese
characters):
錯誤では戻せない
...
Heading (bold, ONE line):
仮登記も列挙にない権利
Diagram: A decision-tree flowchart sharing the same branch structure as
Panel 3's合筆制限の例外を判定する木 (same diamond node: 土地に付いてい
る所有権以外の権利は、不動産登記規則が合筆の例外として限定列挙する権利
(承役地の地役権・条件が同一の担保権など)に含まれるか？). This time the
いいえ branch is drawn with a thick highlighted border pointing to a
labeled box 所有権の移転の仮登記(本肢), leading to a conclusion node
reading 受付年月日・受付番号・登記原因及びその日付が同一でも合筆できな
い. The いいえ側の box for 永小作権・採石権 is drawn faded and
greyed-out, since it belongs to another肢. The はい branch is also drawn
faded and greyed-out, leading to a small conclusion node reading 条件が
同一なら合筆できる場合がある, since this branch is not本肢の対象.
着眼点 callout (1-2 sentences, verbatim, must state the checking order):
まず、所有権の移転の仮登記が、不動産登記規則が合筆の例外として限定的に
認める権利の一覧に含まれているかどうかを確認します。含まれていない場合
は、次に、受付年月日・受付番号・登記原因及びその日付が同一であっても、
その一致だけでは合筆できないと判断します。
Conclusion tag (a short colored banner/pill, blue, 5-15 Japanese
characters):
仮登記も不可
