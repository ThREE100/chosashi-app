...
分筆の登記の添付情報は「分筆後の土地の地積測量図」であり、土地所在図は土地の表題登記のときに提供する図面です（不動産登記令別表8項・4項）。分筆の申請で両図面を並べて提供する場面自体がなく、本肢は前提を欠きます。
...
地積測量図に記録する筆界点の座標値は、基本三角点等に基づく測量の成果によるのが原則です（不動産登記規則77条1項8号）。近傍に基本三角点等が存しない場合など特別の事情があるときは、近傍の恒久的な地物に基づく測量の成果によることができます（同条2項）。
