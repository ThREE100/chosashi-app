...
├── h19-mondai/             平成19年度 午前の部(第1問〜第20問、全問完成。※他年度と部の表記が異なる点に注意、下記参照)
...
## 平成19年度分の出典表記について（「午前の部」「午後の部」の食い違い）

平成19年度分（h19-mondai）は、ユーザーが保存していた法務省公表の平成19年度試験問題原本（スキャンPDF、択一式第1問〜第20問＋記述式第21問・第22問を収録）を主要な情報源として作成しました。

- このPDFの1ページ目には、はっきりと「（19）試験問題（午前の部）」と印字されています。つまり、当該年度の択一式第1問〜第20問・記述式第21問・第22問は、原本上は**午前の部**として出題されたものです（この一冊の中に「午後の部」という別冊は存在しません）。
- 一方、本リポジトリの他年度（平成17年度・平成20〜30年度・令和1〜7年度）の記事は、同じ構成の問題（択一20問＋記述2問）を一貫して「**午後の部**」として出典表記しています。
- `format-template.md` の執筆ルールでは出典（年度・回・部）は法務省原本で確認した情報のみを使うこととされているため、この食い違いをユーザーに確認し、**平成19年度分の記事のみ、原本表記のとおり「午前の部」と記載する**ことで合意しました。
- 他年度が本当に「午後の部」なのか、それとも同様に原本は「午前の部」なのかは、平成19年度分の作成時点では未確認です。今後、他年度の原本を確認する機会があれば、あわせて検証してください（[Issue #4](https://github.com/ThREE100/chosashi-app/issues/4)として起票済み）。

## 平成19年度分の情報ソースについて

- 択一式第1問〜第20問の問題文・肢・組合せ・正解番号は、上記の法務省公表試験問題原本PDFを直接読み取って書き起こしたものと、`src/data/takuitsu.json`収録のH19データ（`correctAnswer`）を突き合わせて確認しました。takuitsu.jsonのH19分は肢のテキストが要約されている（原本の逐語引用ではない）ため、記事本文の引用は原本PDFの書き起こしを優先しています。
- 各肢の法的根拠は、条文(民法・不動産登記法・不動産登記規則・不動産登記事務取扱手続準則・土地家屋調査士法等)をWebSearch・実務解説サイトで確認しています。**この作成セッションでは、e-Gov法令検索(laws.e-gov.go.jp)・法務省公式サイト(moj.go.jp)への直接アクセス(WebFetch等)がネットワーク環境のプロキシポリシーにより403で拒否され、一次資料の条文原文を直接開くことができませんでした。** そのため、条文の内容はWebSearchの検索結果スニペットや複数の独立した逐条解説サイト・予備校サイトの記載が一致することを確認する方法で裏付けており、条文の号番号レベルまで一字一句の原文照合ができた肢と、二次情報源の相互確認にとどまる肢があります。各記事末尾の確認事項ブロックに、肢ごとの確認状況を正直に記載しています。
- 作成後、全20記事について次のダブルチェックを実施しました：(1)各記事の正解番号がユーザー提供の公式正解表と20問すべて一致することを確認、(2)まとめの各肢判定と最終的な正解の組合せが矛盾していないことを確認、(3)各記事の作成時に確度が低いとされた論点のうち特に重要な3点（第1問エの随意条件の停止条件/解除条件による違い、第19問の建物として取り扱わないものの具体的な組合せ、第20問の調査士法人の補助者届出先）を、別途WebSearchで独立に再検証し、いずれも記事の結論を確認しました。
- 第14問（筆界特定手続の流れ図の穴埋め）、第15問（押印の要否）、第16問（一の申請情報による同時申請の可否）、第19問（建物として取り扱うことができないものの個数、ア〜コの10肢）、第20問（調査士・調査士法人に関する誤っているものはどれか）は、他年度に見られる標準的な「ア〜オの正誤の組合せ」形式ではない特殊な出題形式のため、`format-template.md`の構造を保ったまま、各問題の形式に合わせて見出しの立て方をアレンジしています。
