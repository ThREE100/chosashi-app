...
├── infographic-prompt-template.md  問題全体を俯瞰するインフォグラフィック用プロンプトの作成ルール
...
平成25年度分（h25-mondai）の全20記事には、上記に加えて末尾に9番目の要素として「インフォグラフィック プロンプト（問題全体）」を追加しています。`infographic-prompt-template.md` のルールに従い、各問題の5肢（まとめ表の正しいルール）を俯瞰できる画像生成プロンプトを、記事本文とは別セクションとして記載したものです。画像生成そのものはユーザーが手動で行うため、記事には完成したプロンプト文のみを収録しています。
