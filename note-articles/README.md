...
├── infographic-prompt-template.md  問題全体インフォグラフィックのプロンプト作成ルール(ルール②)
...
├── h18-mondai/             平成18年度 午後の部(第1問〜第20問、全問完成)
...
平成18年度分（h18-mondai）は、ユーザーから提供されたアガルートの過去問解説PDF（総則・物権を含む午後の部第1〜20問）を、正解番号・各肢の正誤判定の突合（チェック）用途にのみ使用し、解説文はそのまま転記せず独自に執筆しました。

- 出題年度・問題番号・正解番号は、アガルート教材との突合に加えて、本アプリのデータベース（`src/data/takuitsu.json`。メタデータ上、法務省公式正答を含む出典を掲げる）とも照合し、20問すべてで一致を確認しています。
- 各肢の法的根拠は、WebSearchによるe-Gov法令検索・実務解説サイトの複数ソース突合で可能な限り検証しましたが、この作業環境ではWebFetchツールが利用できず（moj.go.jp・e-Gov等、試行したすべてのサイトで403エラー）、条文原文を直接開いて確認することはできませんでした。断定できなかった条文番号・先例番号は、各記事末尾の確認事項ブロックに正直に記載しています。
- 第3問・第10問は、アガルート教材が問題番号を「問題3改」「問題10改」と表記していました。法務省の原本を本環境から直接確認できなかったため、独自の法的分析（各肢の結論がいずれも出題(2006年)より前の判例で確定していること等）とデータベースとの突合により、少なくとも掲載した正解には影響がないと判断した上で作成しています。詳細は両記事末尾の確認事項ブロックを参照してください。
