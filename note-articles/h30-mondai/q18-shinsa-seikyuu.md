...
- **ア（誤）**　監督法務局長等は自ら処分せず、登記官に相当の処分を命じる
- **イ（誤）**　登記官の処分への審査請求に期間制限はない（審査請求期間を定める規定の適用が除外されているため）
- **ウ（正）**　裁決書の謄本・審理員意見書の写しを審査請求人・登記官に交付
- **エ（正）**　不作為の申請却下は、登記官に却下する処分を命じる
- **オ（正）**　証拠書類は原則として提出者に速やかに返還する
