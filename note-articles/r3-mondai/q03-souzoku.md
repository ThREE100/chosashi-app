...
- 一般原則・判例からの推論にとどまる部分：イ（代襲者と養子の二重資格による相続分の合算で3分の2となる点）は、二重資格者の相続分合算という先例・判例の考え方に基づく結論で、データベース内でも根拠となる先例の表記（昭和26年の先例／判例）に揺れがあり、計算過程は一般原則からの推論を含みます。数値の3分の2という結論自体は検証済みデータベースの判定と一致しています。
