...
- **ア（正）** 閉鎖された地番は原則再使用しない
- **イ（誤）** 地番が著しく錯雑している場合において必要があると認めるときは、登記官は地番を変更できる
- **ウ（誤）** 合筆後の地番は原則として筆頭地番（若番）。10番2とするには特別の事情が必要
- **エ（正）** 同一地番上の複数建物は支号を付けて区別する
- **オ（正）** 家屋番号は管轄指定を受けた登記所の地番による
