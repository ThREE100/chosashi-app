...
仮換地として指定された土地上に建物を新築し、その建物表題登記を申請する場合、提供すべき建物図面には、仮換地の形状及び当該建物の位置を「実線」で図示しなければなりません（不動産登記規則83条2項）。従前地の形状や位置のほうを点線で図示します。肢イは仮換地を「点線」で図示するとしており、実線と点線が逆になっているため誤りです。
...
| イ | 誤 | 仮換地は実線・従前地は点線で図示する。実線と点線が逆（規則83条2項） |
...
- 各肢の根拠のうち、条文・規則レベルまで確認できているのは、ア（不動産登記令別表14項添付情報イ）、イ（不動産登記規則83条2項＝仮換地を実線・従前地を点線で図示）、ウ（不動産登記規則88条2項）、オ（不動産登記令別表16項添付情報イ）です。イについては、検証済みデータのOCR部分に先例（昭和40年4月10日民甲837号回答）への言及もありますが、条文の裏付けとしては規則83条2項を根拠に記載しています。
- 一方、エ（団地共用部分である旨の規約を廃止したことによる表題登記の申請では建物図面・各階平面図の提供を要しないこと）については、「すでに登記記録に図面が備えられているため改めての提供は不要」という一般原則・実務上の取扱いからの説明にとどまり、個別の条文番号までは検証済みデータ上で特定できていません。
- なお、アガルート等のローカル教材PDFは本執筆環境に存在せず、参照していません。上記の根拠はすべて検証済みデータベースの記載に基づくものです。
