...
- **イ（押印不要）**　委任による代理人が申請書に署名した場合は押印を要しない
- **ウ（押印不要）**　図面への申請人の記名は、そもそも押印を求める規定がない
- **エ（押印不要）**　署名した委任状に公証人の認証を受けた場合は押印を要しない
- **オ（押印不要）**　作成者は署名又は記名押印の選択制であり、署名を選んだ以上押印は不要
...
- **QAチェックリスト再検証（2026-09-19実施）**：`note-articles/qa-checklist.md`の全19項目に基づき再度フルスキャンしました。不動産登記令16条1項・18条1項（`fudousan-touki-rei.md`）、不動産登記規則47条1号・49条1項1号・74条2項（`fudousan-touki-kisoku-1.md`）の条文原文を再度Grep・Readで一字一句突合し、いずれも本文の引用と完全に一致することを確認しました。正解番号（1番＝1個）を`src/data/takuitsu.json`の`chosashi_H19_q15`の`correctAnswer`と再照合し一致を確認しました。判例・先例・専門誌番号が本文にないこと、全角括弧統一、敬体統一、見出しの正誤判定語なし、正解の先出しなし、Markdown表不使用、コードフェンス数の偶数性も確認しました。インフォグラフィックプロンプトのカード内容（記名・署名・公証人認証・図面申請人欄・作成者の扱い）は現在の本文と齟齬なく整合しています。修正が必要な誤りは見つからず、本文・まとめ・正解番号・確認事項の記載を変更する必要はありませんでした。
...
「委任状・申請書」は原則どおり記名押印を要するが例外があるという3枚と、「添付図面」はそもそも申請人の押印を求めていないという2枚を2列に分け、通し番号バッジ・短い見出し・図解・短い結論タグだけで構成する計5枚のポスター型カードで俯瞰する構成。
...
seals, pens for handwritten signatures, notary stamps, land survey
diagrams, floor plans, checkmarks, prohibition marks — adapt icon set to
the topic).
...
kanji 押・印・署・名・委・任・図・面・作・成・者・登・記・証・請・認 —
...
記名・署名・認証で変わる押印の要否（平成19年度 午前の部 第15問）
...
「記名」だけでは押印が必要
Illustration: An isometric delegation document (委任状) on a desk with a
printed/typed name label reading 「申請人　Ａ」 (no handwritten mark), a
red arrow pointing to an empty round hanko-seal impression outline, and a
red stamp reading 「押印必要」 pressed firmly onto the document.
...
Heading (bold, ONE line, ~20 characters or fewer):
代理人の署名なら押印不要
Illustration: An isometric application document (申請書) where a land
surveyor（土地家屋調査士、代理人）signs the document by hand with a pen
(署名), a green checkmark stamp reading 「押印不要」, and a small hanko
seal icon with a red X mark over it showing no stamp is needed.
Conclusion tag (blue banner below the illustration, 5-15 characters):
代理人の署名で足りる
...
Heading (bold, ONE line, ~20 characters or fewer):
署名と公証人認証で不要
Illustration: An isometric delegation document (委任状) signed by hand by
the applicant, next to a notary public（公証人）stamping an official
「認証」 seal onto the document, a green checkmark stamp reading
「押印不要」, and a small hanko seal icon with a red X mark over it.
Conclusion tag (blue banner below the illustration, 5-15 characters):
認証があれば足りる
...
Heading (bold, ONE line, ~20 characters or fewer):
図面の申請人欄は記名で足りる
Illustration: An isometric building floor plan (建物図面) with a small
name-tag box reading 「申請人　Ａ」 printed (not signed), a green
checkmark stamp reading 「押印不要」, and a small hanko seal icon with a
red X mark over it next to the name-tag box.
...
Heading (bold, ONE line, ~20 characters or fewer):
作成者も署名なら押印不要
Illustration: An isometric land survey diagram (地積測量図) with a land
surveyor（作成者）signing the 作成者 signature box by hand with a pen, a
green checkmark stamp reading 「押印不要」, and a small hanko seal icon
with a red X mark over it.
Conclusion tag (green banner below the illustration, 5-15 characters):
作成者は署名でも可
