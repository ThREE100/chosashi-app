## 【土地家屋調査士受験生向け】平成19年度 第14問〜意見提出は聴取期日のあとなんです〜
...
- 出題年度・問題番号（平成19年度午前の部第14問）と正解番号（2番＝筆界調査委員の指定）は、ユーザー提供の資料に基づいて記載しています。`src/data/takuitsu.json`の該当エントリ（`chosashi_H19_q14`）は問題文・選択肢が「Unable to retrieve」となっており`correctAnswer`も未確定（0）のため、データベース側では裏付けが取れませんでした。この点は投稿前にユーザー側で法務省公表の正答資料との最終照合を推奨します。
- 条文レベルで確認できた事項：不動産登記法132条1項（却下事由の審査・却下）、133条（申請があった旨の公告及び通知）、134条（筆界調査委員の指定）、135条・136条（筆界調査委員による事実の調査・測量及び実地調査）、140条（意見聴取等の期日）、142条（筆界調査委員による意見の提出）、143条1項（筆界特定）、144条（筆界特定をした旨の公告及び通知）。`note-articles/laws/fudousan-touki-hou.md`収録の条文原文で条番号・見出し・内容を直接突合し、確認しました。
...
筆界特定手続の「申請→審査→（却下されなければ）公告・通知→筆界調査委員の指定→事実の調査→意見聴取等の期日→意見提出→筆界特定」という一本道の流れを、通し番号バッジ・短い見出し・図解・短い結論タグだけで構成する、計5枚のカードが縦一列に並ぶポスター型で俯瞰する構成。
...
desks, document stamps, magnifying glasses, calendars, professional
figures with name badges, speech bubbles, arrows — adapt icon set to the
topic).
...
まず却下事由がないか審査
Illustration: An isometric registration office desk where a registrar
figure examines an application document with a magnifying glass, next to
a small checklist showing icons for 管轄・申請権限・記載事項. A branching
arrow leads down-and-right toward Card 2, and down toward Card 3.
...
Heading (bold, ONE line, ~20 characters or fewer):
却下事由があれば手続終了
Illustration: A red stamp reading 「却下」 pressed onto the application
document, with a red X mark on the branch path showing this route
terminates and does not continue to the cards below.
...
with speech bubble icons above them.
Conclusion tag (blue banner below the illustration, 5-15 characters):
意見聴取等の期日
...
Heading (bold, ONE line, ~20 characters or fewer):
聴取の後に委員が意見を提出
Illustration: The badged professional figure handing a document stamped
「意見書」 to the registrar's desk, with an arrow pointing onward to a
final stamp reading 「筆界特定」.
Conclusion tag (blue banner below the illustration, 5-15 characters):
筆界調査委員の意見提出
...
Subtitle (smaller, centered, 1行):
平成19年度 午前の部 第14問 ア〜オ作図ガイド（筆界特定手続の流れ）
