## 【土地家屋調査士受験生向け】平成24年度 第17問〜各階平面図、500分の1縛りじゃないんです〜
...
建物図面及び各階平面図は、1個の建物（附属建物があるときは、主である建物と附属建物とを合わせて1個の建物とする。）ごとに作成しなければなりません（不動産登記規則81条）。

**たとえば**、居宅（主である建物）と物置（附属建物）がある場合、建物図面・各階平面図は居宅だけを対象にするのではなく、物置も含めた1個の建物として作成します。

### イ：書面作成の場合の線の太さは、0.3ミリメートルではなく、より細い線が求められる

建物図面及び各階平面図を書面で作成する場合の線の太さについて、「0.3ミリメートル以下」という数値は正確ではありません。不動産登記規則76条1項は、これらの図面（書面である場合に限る）を「〇・二ミリメートル以下の細線により、図形を鮮明に表示しなければならない」と定めており、実際にはより細い線（0.2ミリメートル以下）が求められています。
...
不動産登記規則83条2項は、「各階平面図は、二百五十分の一の縮尺により作成しなければならない。ただし、建物の状況その他の事情により当該縮尺によることが適当でないときは、この限りでない」と定めています。250分の1が適当でない場合について「500分の1の縮尺による」という限定は条文上どこにも書かれておらず、状況に応じた適当な縮尺を選択することができます（なお、規則82条3項は建物図面自体の原則縮尺を500分の1と定めており、各階平面図の縮尺と混同しないよう注意が必要です）。
...
附属建物の新築による建物の表題部の変更の登記を申請する際に提供すべき建物図面は、新築された附属建物の部分だけでなく、主である建物も含めて記録しなければなりません。建物図面は、常に1個の建物（主である建物＋附属建物）全体を表示するものだからです（不動産登記規則81条）。

**たとえば**、既存の居宅に新しく物置を建て増しして附属建物とする変更登記を申請する場合、提出する建物図面には、新築した物置の部分だけでなく、もとからある居宅の部分も含めて描く必要があります。

### まとめ

- **ア（正）**　建物図面・各階平面図は、主である建物と附属建物を合わせて1個の建物ごとに作成する（不登規則81条）
- **イ（誤）**　書面作成時の線の太さは「0.3ミリメートル以下」ではなく「0.2ミリメートル以下」（不登規則76条1項）
- **ウ（正）**　極めて小さくなる建物は、位置のみ記入して余白に拡大表示できる（要外部確認）
- **エ（誤）**　各階平面図の縮尺は、250分の1が適当でない場合でも「500分の1」に限定されない（不登規則83条2項）
- **オ（正）**　附属建物新築による変更登記でも、建物図面は主である建物も含めて記録する（不登規則81条）
...
- `note-articles/laws/`のローカル法令データベース（不動産登記規則、e-Gov現行法2026-08-04取得）で、ア・オ＝規則81条、イ＝規則76条1項（「〇・二ミリメートル以下」と条文原文で確認済み）、エ＝規則83条2項の条文原文を確認しました。特にイは本記事の初版時点で「0.2ミリメートル以下」を確定的に確認できていませんでしたが、今回の照合で解消しています。ウ（極めて僅少な建物の位置のみ記入・余白拡大表示）については、規則76条〜85条を通読して探しましたが、該当する条文をローカル法令データベース内では見つけられませんでした。不動産登記事務取扱手続準則52条・53条付近（図面の記載方法に関する規定）に定めがある可能性がありますが、当該箇所は画像・図表を含む条文で本記事作成時点では完全にはテキスト化できていないため、noteに掲載する前にe-Gov法令検索等での再確認を推奨します。
