...
建物図面及び各階平面図を書面で作成する場合、0.2ミリメートル以下の細線により図形を鮮明に表示しなければなりません（不動産登記規則73条1項）。「0.3ミリメートル以下」という数値は誤りです。
...
各階平面図は、250分の1の縮尺により作成するのが原則ですが、建物の状況その他の事情によりこの縮尺によることが適当でないときは、この限りではありません（不動産登記規則83条2項）。「500分の1の縮尺」に限定する規定はなく、状況に応じた適当な縮尺を選択することができます。
...
- **ア（正）**　建物図面・各階平面図は、主である建物と附属建物を合わせて1個の建物ごとに作成する（規則81条）
- **イ（誤）**　書面作成時の線の太さは「0.3ミリメートル以下」ではなく、0.2ミリメートル以下（規則73条1項）
- **ウ（正）**　極めて小さくなる建物は、位置のみ記入して余白に拡大表示できる
- **エ（誤）**　各階平面図の縮尺は、250分の1が適当でない場合でも「500分の1」に限定されない（規則83条2項）
...
- 各肢の根拠は、`note-articles/laws/fudousan-touki-kisoku-1.md`の条文原文で確認済みです。ア＝規則81条（作成単位）、イ＝規則73条1項（線の太さ0.2ミリメートル以下）、エ＝規則83条2項（各階平面図の縮尺250分の1、ただし書）です。ウ（僅少な建物の拡大表示）とオ（附属建物新築時の記録範囲）については、`laws/`収録法令中に本問の具体的表現と完全に一致する条文を特定できず、実務解説からの整理にとどまります。
- **適用法令の現行性チェック（2026-08-06実施）**：`note-articles/laws/`のローカル法令データベース（不動産登記規則、2026-08-04取得の現行有効版）で第73条・81条・83条を確認しました。特にイについては、初版で「0.2ミリメートル以下」を未確定として保留していましたが、規則73条1項の条文原文「〇・二ミリメートル以下の細線により、図形を鮮明に表示しなければならない」で確定させました。平成24年の出題後の改正でも各条文の内容に変更はなく、各肢の結論・正解（4番＝イ・エ）に変更はありません。
