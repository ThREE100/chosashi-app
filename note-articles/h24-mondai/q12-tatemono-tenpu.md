...
- **ア（誤）**　共用部分の更正登記でも、所有者を証する情報の提供は省略できない（不登令別表20の項ロ）
- **イ（正）**　相続による表題登記でも、相続を証する市町村長作成の情報の提供が必要（不登令別表12の項ト）
- **ウ（正）**　抵当証券が発行されていない抵当権があるときは、登記名義人の承諾証明情報が必要（不登令別表18の項ロ）
- **エ（誤）**　規約廃止後の表題登記でも、所有権を証する情報の提供は省略できない（不登令別表21の項ロ）
- **オ（誤）**　同一管轄区域内の土地であれば、登記事項証明書の提供は不要（不登令別表15の項ホ(2)の反対解釈）
