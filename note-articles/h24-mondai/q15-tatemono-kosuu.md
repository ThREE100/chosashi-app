...
- **ア（誤）**　効用上一体として利用されていなければ、数棟をまとめて1個の建物として登記できない（準則78条1項）
- **イ（正）**　階段室・エレベーター室などの法定共用部分は、独立した登記の対象にならない（区分所有法4条1項）
- **ウ（正）**　物理的な現況が変わらなくても、登記の申請によって建物の個数は増減する（不登法54条、準則78条2項）
- **エ（誤）**　区分建物の登記記録は、1棟ごとではなく専有部分ごとに作成される（不登法2条5号・22号）
- **オ（誤）**　構造上区分された部分でも、所有者が同一であれば1個の建物として登記できる（準則78条2項ただし書）
