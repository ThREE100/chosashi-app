...
- 各選択肢の判定根拠は、`note-articles/laws/kubunshoyuu-hou.md`の条文原文で確認済みです。区分所有法32条は「最初に建物の専有部分の全部を所有する者は、公正証書により、第四条第二項、第五条第一項並びに第二十二条第一項ただし書及び第二項ただし書…の規約を設定することができる」と定めており、公正証書による単独設定の対象を①4条2項（規約共用部分）、②5条1項（規約敷地）、③22条1項ただし書（分離処分を可能にする定め）、④22条2項ただし書（敷地利用権の持分割合の別段の定め）の4つに限定列挙しています。選択肢1〜5はこの4事項＋列挙外の事項（法定共用部分の持分割合の変更）を並べたもので、条文の構造と完全に一致します。
- **適用法令の現行性チェック（2026-08-06実施）**：`note-articles/laws/`のローカル法令データベース（区分所有法、2026-08-04取得の現行有効版）で第32条・4条・5条・22条の条文原文を確認し、上記のとおり32条の限定列挙構造を確定させました。令和7年法律第47号（老朽化マンション等の管理及び再生の円滑化等を図るための建物の区分所有等に関する法律等の一部を改正する法律、2026年4月1日施行）による改正は、集会決議要件の緩和やマンション再生手法の拡充が中心で、`laws/`に収録されている現行の32条・4条・5条・22条の内容にも改正の影響は反映されておらず、本問の各肢の結論に変更はありません。正解（3番）に変更はありません。
