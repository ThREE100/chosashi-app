...
- **適用法令の現行性チェック（2026-08-04実施）**：2026年8月時点の現行法で再確認しました。床面積の算定基準を定める不動産登記規則115条は条文番号・内容とも変更されていません。なお同条は、区分建物については「壁その他の区画の内側線」で囲まれた部分によるとする括弧書きを置いています（一般の建物は中心線で算定します）。本問の各肢はいずれも一般の建物を前提とした出題であり、各肢の算入可否の実務基準についても、これを変更する改正・先例は確認されず、結論に変更はありません。
