...
- 各肢の根拠として挙げた条文は、ア（不動産登記法47条1項）・イ（同法58条6項）・ウ（同法30条）・エ（同法51条）・オ（同法30条）です。イの根拠条文（共用部分である旨の登記がある建物の規約廃止に伴う表題登記申請義務）は58条6項として記載していますが、条番号の細部は各自e-Gov法令検索で最終確認されることをおすすめします（○×判定自体は公式正答で確認済み）。
