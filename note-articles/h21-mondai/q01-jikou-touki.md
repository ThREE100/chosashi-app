...
- **適用法令の現行性チェック（2026-08-04実施）**：2026年8月時点の現行法で再確認しました。2020年4月施行の民法（債権関係）改正は消滅時効の期間・起算点の整理（「中断」→「更新」等の用語整理を含む）が中心で、所有権の取得時効（民法162条）・対抗要件（民法177条）の規定そのものに実質的な改正はなく、時効完成の前後で第三者への対抗に登記の要否が分かれるとする判例法理（起算点固定を含む）も維持されています。各肢の結論に変更はありません。
