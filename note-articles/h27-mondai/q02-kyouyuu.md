...
- **ア（正）**　不実登記の抹消請求は自己の持分の範囲に限られる
- **イ（誤）**　過半数持分でも占有する第三者に当然の明渡しは請求できない
- **ウ（誤）**　不法占有の損害賠償は自己の持分に応じた部分のみ
- **エ（誤）**　無断の変更に対し各共有者は単独で原状回復を請求できる
- **オ（正）**　賃貸借の解除は管理行為で持分の過半数で決する
...
- 各肢の判例（ア＝最判平15・7・11、イ＝最判昭41・5・19、ウ＝最判昭41・3・3および最判昭51・9・7、エ＝最判平10・3・24、オ＝民法252条1項・最判昭39・2・25）は、データベースのexplanation欄に明記されているものを用いています。オについては契約解除の不可分性（民法544条1項）との関係を補足していますが、これは一般原則からの説明です。
- **現行法チェック（民法条文原文で確認）**：オの根拠条文について、当初は「民法252条本文」としていましたが、令和3年民法改正（令和5年4月1日施行）により252条は1項〜5項に再構成され、旧法にあった「本文／ただし書」という条文構造自体が現行法には存在しません（現行252条1項に「ただし書」はなく、単独でできる保存行為は新設の5項に独立して規定されています）。共有物の管理事項を持分の過半数で決するという本肢の結論の根拠は、現行法では「252条1項」と引用するのが正確なため修正しました。結論自体は改正の前後で変わりません。
