...
- **イ（正）**　押印は書面を作った代理人のもの、本人（未成年者）の押印は不要
- **ウ（誤）**　氏名・住所は登記を受ける本人を示す情報のため省略不可
- **エ（誤）**　法人は代表者の「氏名」を記載、住所は不要
- **オ（正）**　代表取締役が複数でも一人を記載すれば足りる

「書面を作ったのは誰か（押印）」と「登記を受けるのは誰か（氏名・住所）」を切り分けると、押印や記載のルールがすっきり整理できます。
...
- 各肢の根拠のうち、ア（不動産登記令16条1項）、エ（不動産登記令3条2号）、オ（会社法349条4項）はデータベースのexplanation欄で条文番号まで明記されているものです。イ・ウ（代理申請における本人の押印は不要だが氏名・住所の記載は必要）は、代理人による申請の一般的な取扱いに基づく説明です。
- **オの根拠条文の訂正**：オの根拠として当初「会社法349条2項」を挙げていましたが、同項は「代表取締役を定めていない場合に、取締役が二人以上あるときは各自が代表する」という規定であり、本肢のように代表取締役Ａ・Ｂが選定済みの場面には当てはまりませんでした。正確な根拠は同条4項（代表取締役は業務に関する一切の裁判上又は裁判外の行為をする権限を有する）で、代表取締役が複数いてもその権限を各自が単独で有するという理解の根拠になります。本文・確認事項ブロックとも修正しました。結論（オは正しい）自体は変わりません。
...
- **説明の補足（初学者向け）**：イ（押印は不要）とウ（氏名・住所は必要）は、一見すると矛盾しているように見えて混乱しやすい箇所です。押印は「書面を作成した人（代理人）」の確認、氏名・住所の記載は「登記を受ける人（本人）」の表示という、別々の役割を担っていることを本文・まとめ・インフォグラフィックに明記し、両者の結論が分かれる理由が分かるように補足しました。
- **重複出題チェック（2026-07-20実施、2026-08-16追記）**：takuitsu.jsonを検索し、平成27年度より後に実施された試験（平成28年度〜令和7年度、2026年7月時点でデータベースに存在する全193問）で、本問（合体等の書面申請の押印・代理申請時の押印省略・法人代表者の記載）と同一・類似の問題が再出題されていないかを確認しました。「申請情報及び添付情報」という広いテーマの問題は平成30年度第4問・令和6年度第4問などで繰り返し出題されていますが、内容は共通添付情報の援用・資格証明書の有効期間・法定相続情報一覧図の利用・電子署名など、本問が扱う書面申請の押印可否や法人代表者の記載事項とは異なる具体的論点でした。ただし、エ肢（法人の表題登記における代表者の氏名記載の要否）と同一論点は、平成29年度第6問イ（会社法人等番号を提供した場合でも代表者の氏名を申請情報の内容とすることを要するか）でも問われており、この点については重複があります（`h29-mondai/q06-shinsei-jouhou.md`）。令和8年度以降の試験がデータベースに追加された際は、この確認を再実施してください。
...
labeled "本人の押印". Beside the parent figure, a small torn-paper label
reads "押すのは代理人" to show the seal belongs to whoever actually
prepared and submitted the document.
Conclusion tag (green banner below the illustration, 5-15 characters):
本人の押印不要
...
also filled in with a green checkmark, showing it is not left blank. Beside
the "本人" box, a small torn-paper label reads "氏名住所＝本人の表示" to
show this states who the registration is actually for, regardless of who
submits it.
...
Subtitle (smaller, centered, 1行):
平成27年度 午後の部 第4問 作図ガイド（申請情報と押印のルール）
