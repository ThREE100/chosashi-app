...
- 備考: 条文中の「第◯条」の条番号は、原文の漢数字表記から算用数字表記に変換して見出し化した（例：「第百七条」→「### 第107条」）。項番号（原文では全角数字１２３…）は「**1**」のように半角太字に変換した。号（一、二、三…）はそのまま漢数字表記を保持している。章・節・款の見出しレベルは、章＝##、節＝###、款＝####、条＝#####とした。この法律ファイルは、本プロジェクトの`laws/`フォルダに従前存在しなかったため、今回新規に追加した（既存の複数のnote記事――`topics/jyuzenchi-sokochi-karikanchi.md`、`h17-mondai/q11-kukakuseiri.md`、`h18-mondai/q08-daii-touki.md`、`r1-mondai/q14-daii-shinsei.md`、`topics/daii-shinsei.md`等――が、土地区画整理法の条文原文をこのプロジェクト内で確認できないと確認事項欄に記載していた）。第82条（土地の分割及び合併＝施行者が代位して分筆・合筆の手続をできる根拠）、第98条（仮換地の指定）、第103条・第104条（換地処分・その効果）、第107条（換地処分に伴う登記等。2項が施行者の申請又は嘱託、3項が他の登記の禁止、4項が不動産登記法の特例の政令委任）、第108条（保留地等の処分）等は、既存記事の条文引用の裏付けとして特に参照価値が高い。
