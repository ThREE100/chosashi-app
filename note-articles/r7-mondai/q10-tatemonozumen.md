## 【土地家屋調査士受験生向け】令和7年度 第10問・建物図面・各階平面図、いつ出せばいい？いつ省略できる？〜所在の変更・規約敷地の追加・仮換地の図示・区分建物の合併・共用部分の規約廃止〜
...
- **ア（誤）**　建物の所在変更では建物図面は必要、各階平面図のみ不要（令別表14の項添付情報欄イ）
...
--- COLUMN A, CARD 1 ---
Heading (bold):
所在が変わっても、いらないのは各階平面図だけ（建物図面は必要）
...
another (地番表示が変わる様子を示す看板アイコン). Two document icons are
shown side by side: one labeled「建物図面」has a green "○" stamp overlaid
on it to show it is required, and one labeled「各階平面図」has a red "×"
stamp overlaid on it to show it is not required.
Caption (small text below):
建物の所在（地番等の表示）が変わるときは、変更後の建物図面の提出が必要
（不動産登記令別表14の項添付情報欄イ）。ただし、建物内部の各階の形状自体
は変わらないため、各階平面図の提出は不要。
