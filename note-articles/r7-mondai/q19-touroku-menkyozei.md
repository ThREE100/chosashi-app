## 【土地家屋調査士受験生向け】令和7年度 第19問〜3,000円に見えても、実は2,000円なんです〜
...
--- HEADER ---
Title (large, bold):
数えるのは「登記の種類」ではなく「建物の個数」

Subtitle (smaller, centered):
令和7年度 午後の部 第19問 オ－なぜ2,000円になるのか

--- COMPARISON ---
LEFT PANEL, heading (bold):
誤った数え方
Illustration: Three separate isometric building icons lined up, each with
its own price tag reading 1,000円: (1) a building labeled 甲建物（分割後）,
(2) a small separate building labeled 切り離された附属建物, drawn as if it
independently exists on its own, (3) a building labeled 乙建物（合併後）.
An arrow points from all three down to a total sum bubble reading 3,000円.
Panel takeaway (short line below the illustration):
分割で2個、合併で1個、合計3個と数えてしまう

RIGHT PANEL, heading (bold):
正しい数え方
Illustration: Two isometric building icons only: 甲建物（附属建物が外れて
小さくなる、1,000円のタグ付き）と乙建物（附属建物が増えて大きくなる、
1,000円のタグ付き）。その間に、切り離された附属建物が独立した建物になる
