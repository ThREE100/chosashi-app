## 【土地家屋調査士受験生向け】令和4年度 第11問〜家屋番号は登記所が決める、変えられないんです〜
...
- **ア（正）** 区分建物の非区分附属建物の所在は構造欄に記録
- **イ（誤）** 増築で土地にまたがったら所在欄に地番を加える変更登記ができる
- **ウ（誤）** またがる建物は床面積の多い方の地番を先に記録
- **エ（誤）** 家屋番号は登記所が定め、申請人は変更登記を申請できない
- **オ（正）** えい行移転後は甲・乙いずれの登記所にも申請できる
...
- **再検証（2026-08-16実施）**：`note-articles/laws/`配下の条文原文と本文を直接照合しました。ア（不動産登記規則4条3項別表3＝別表三「附属建物の表示欄」の構造欄）、ウ（準則88条2項「床面積の多い部分…の所在する土地の地番を先に記録し」。同条3項の記載例「６番地、４番地、８番地」とも整合）、エ（不動産登記法45条）、オ（準則4条1項・2項）は、いずれも条文原文レベルで確認できました。一方、イの根拠として挙げている「昭59質疑応答」はローカル法令データベースに条文としては収録されていない実務上の質疑応答であり、条文そのものでは確認できていません（本文には具体的な先例番号ではなく年と種別のみを記載し、番号は明示していません）。正誤判定・本文の記述に変更はありません。
- **重複出題チェック（2026-08-16実施）**：他年度の類似論点をtakuitsu.jsonおよびnote-articles/配下で検索しました。ウ（2筆にまたがる建物は床面積の多い部分の地番を先に記録、準則88条2項）は、平成25年度午後第10問オ（`note-articles/h25-mondai/q10-tatemono-shozai.md`）とほぼ同一の論点です。ただし挙げている具体例の地番が異なり、内容も矛盾しないため、両記事とも修正の必要はありません。ア・イ・エ・オについては、他年度での同一・類似論点の出題は確認できませんでした。なお、アの「区分建物の附属建物の所在は構造欄に記録される」というテーマは、令和4年度午後第14問（附属建物が区分建物である場合の発展論点）や`note-articles/topics/fuzoku-tatemono-shozai.md`と関連しており、既に相互参照が取れていることを確認しました。
- **最新法令チェック（2026-08-16実施）**：相続登記の申請義務化（令和6年4月1日施行）・住所等変更登記の申請義務化（令和8年4月1日施行）は、いずれも権利部（所有権の登記名義人の氏名・住所）に関する申請義務の改正であり、本問が扱う建物の所在・家屋番号の記録方法（表示に関する登記の手続自体）には影響しません。各肢の結論に変更はありません。
...
Subtitle (smaller, centered, 1行):
令和4年度 午後の部 第11問 作図ガイド（建物の所在欄と家屋番号）
