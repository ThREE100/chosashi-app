...
- **ア（誤）** 共有持分権を敷地権として丁区分建物の表題登記を申請できる
- **イ（正）** 規約敷地になったらBは1か月以内に表題部変更登記を申請
- **ウ（誤）** 規約設定には集会出席者の4分の3以上の多数決が必要でAのみでは不可
- **エ（正）** 一部の区分建物についてのみ分離処分可能規約を設定できる
- **オ（誤）** 分離処分には前提として敷地権登記の抹消が必要
